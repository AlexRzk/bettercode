import type { Config, BettercodeClient, Path, Project, ProviderAuthResponse, Todo } from "@bettercode/sdk/v2/client"
...
import { createSimpleContext, NormalizedProviderListResponse } from "@bettercode/ui/context"
...
export const loadMcpQuery = (scope: ServerScope, directory: string, sdk: BettercodeClient) =>
  queryOptions({
    queryKey: [scope, directory, "mcp"] as const,
    queryFn: () => sdk.mcp.status().then((r) => r.data ?? {}),
  })

export const loadLspQuery = (scope: ServerScope, directory: string, sdk: BettercodeClient) =>
  queryOptions({
...
function makeQueryOptionsApi(
  ...
  sdkFor: (dir: PathKey) => BettercodeClient,
) {
  return {
...
export function createServerSyncContextInner(serverSDK: ServerSDK) {
  ...
  const sdkCache = new Map<string, BettercodeClient>()
