  ...
  test("moves an existing canonical web bucket into local scope", () => {
    expect(
      migrateCanonicalLocalServerState(
        {
          list: [],
          projects: { "https://bettercode.example.com": [{ worktree: "/remote", expanded: true }] },
          lastProject: { "https://bettercode.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://bettercode.example.com"),
      ),
    ).toEqual({
      list: [],
      ...
      migrateCanonicalLocalServerState(
        {
          projects: {
            ...
            "https://bettercode.example.com": [
              { worktree: "/local", expanded: true },
              { worktree: "/remote", expanded: true },
            ],
          },
          lastProject: { local: "/local", "https://bettercode.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://bettercode.example.com"),
      ),
    ).toEqual({
