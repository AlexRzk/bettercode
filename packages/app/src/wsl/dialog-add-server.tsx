import { Button } from "@bettercode/ui/button"
import { useDialog } from "@bettercode/ui/context/dialog"
import { Spinner } from "@bettercode/ui/spinner"
import { showToast } from "@bettercode/ui/toast"
...
type WslServerStep = "wsl" | "distro" | "bettercode"

const STEPS: WslServerStep[] = ["wsl", "distro", "bettercode"]
...
export function DialogAddWslServer(props: DialogWslServerProps = {}) {
  ...
  const bettercodeCheck = createMemo(() => {
    const distro = selectedDistro()
    if (!distro) return null
    return current()?.bettercodeChecks[distro] ?? null
  })
  ...
  const bettercodeReady = createMemo(() => {
    const check = bettercodeCheck()
  ...
  const installingOpencode = createMemo(() => {
    ...
    return job?.kind === "install-bettercode" && job.distro === selectedDistro()
  })
  const allReady = createMemo(() => wslReady() && distroReady() && bettercodeReady())
  const addDisabled = createMemo(() => {
    const job = current()?.job
    if (!job) return store.adding
    return store.adding || job.kind !== "probe-bettercode"
  })
  ...
  const autoProbe = createMemo(() => {
    ...
    if (!state.bettercodeChecks[distro]) {
      return { key: `probe-bettercode:${distro}`, run: () => api.probeOpencode(distro) }
    }
    return null
  })
  ...
  const bettercodeMessage = createMemo(() => {
    ...
    if (bettercodeReady()) {
      return distro
        ? language.t("wsl.onboarding.bettercodeReadyIn", { distro })
        : language.t("wsl.onboarding.bettercodeReady")
    }
    return distro
        ...
        title:
          step === "wsl"
            ...
            : step === "distro"
              ...
              : language.t("wsl.onboarding.step.bettercode"),
        state:
          active === step
            ? "current"
            : step === "wsl"
              ...
              : step === "distro"
                ? distroReady()
                  ...
                  : index > activeIndex
                    ? "locked"
                    : "warning"
                : bettercodeCheck()?.matchesDesktop === false
                  ? "warning"
                  : bettercodeReady()
                    ? "done"
                    : index > activeIndex
      ...
      >
        ...
        >
          ...
          <Switch>
            ...
            <Match when={activeStep() === "bettercode"}>
              <div class="rounded-md bg-surface-base p-4 flex flex-col gap-3">
                <div class="flex items-center justify-between gap-3">
                  <div class="text-14-medium text-text-strong">{language.t("wsl.onboarding.step.bettercode")}</div>
                  <div class="flex items-center gap-2">
                    <Show when={selectedDistro()}>
                      <Button
                        variant="ghost"
                    ...
                    <Show when={!bettercodeReady() || bettercodeCheck()?.matchesDesktop === false}>
                      <Button
                        variant="secondary"
                        size="large"
                        disabled={busy()}
                ...
                <div class="text-12-regular text-text-weak whitespace-pre-wrap break-words">{bettercodeMessage()}</div>
                <Show when={bettercodeCheck()?.matchesDesktop === false ? bettercodeCheck() : null}>
                  {(check) => (
                    <div class="rounded-md border border-border-weak-base px-3 py-3 flex flex-col gap-1">
                      <div class="text-12-regular text-text-weak">
          ...
          <Show when={activeStep() === "bettercode" && allReady() && selectedDistro()}>
