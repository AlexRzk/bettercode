...
describe("WSL server settings presentation", () => {
  ...
  test("offers install and update only when BetterCode needs attention", () => {
    ...
    ).toBe("Install BetterCode")
    expect(
      wslOpencodeAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/bettercode",
        version: "1.2.2",
        expectedVersion: "1.2.3",
        matchesDesktop: false,
        error: null,
      }),
    ).toBe("Update BetterCode")
    expect(
      wslOpencodeAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/bettercode",
  ...
  test("probes the selected distro before entering the BetterCode step", async () => {
    const calls: string[] = []
    ...
    expect(calls).toEqual(["Debian", "bettercode"])
  })
})
