...
import { Tag } from "@bettercode/ui/v2/badge-v2"
import { ButtonV2 } from "@bettercode/ui/v2/button-v2"
import { Dialog } from "@bettercode/ui/v2/dialog-v2"
    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.bettercodeChecks[item.config.distro]
          const bettercodeAction = () => wslOpencodeAction(check())
          const busy = () => wsl.data?.job?.kind === "install-bettercode" && wsl.data.job.distro === item.config.distro
          return (
