...
const pluginReExport = `// Auto-generated by bettercode sync. Do not edit.
...
async function cmdSetup() {
  ...
  if (doPlugin) {
    console.log("Creating .opencode/plugin/bettercode.ts...")
    await cmdSync()
  }
...
async function cmdSync() {
  const bcConfigPath = join(root, configDir, "bettercode.jsonc")
  const pluginFile = join(root, ".opencode", "plugin", "bettercode.ts")
  ...
  // Write plugin re-export file
  writeFileSync(pluginFile, pluginReExport)
  ...
  if (pluginOptions && Object.keys(pluginOptions).length > 0) {
    if (existsSync(opencodeConfig)) {
      try {
        ...
        const pluginEntry = ["./plugin/bettercode.ts", pluginOptions]
  ...
  if (flags.json) {
    ...
    console.log(`  Created .opencode/plugin/bettercode.ts`)
    if (pluginOptions && Object.keys(pluginOptions).length > 0) {
...
async function cmdDoctor() {
  ...
  // Check 1: OpenCode CLI package exists
  ...
  if (existsSync(cliPackagePath)) {
    try {
      ...
      checks.push({ name: "OpenCode CLI", status: "ok", detail: `${pkg.name}@${pkg.version}` })
    } catch {
      checks.push({ name: "OpenCode CLI", status: "error", detail: "Invalid package.json" })
    }
  } else {
    checks.push({ name: "OpenCode CLI", status: "error", detail: "packages/cli/package.json not found" })
  }

  // Check 2: @bettercode/plugin workspace link
  const pluginPackagePath = join(root, "packages", "bettercode-plugin", "package.json")
  if (existsSync(pluginPackagePath)) {
    checks.push({ name: "@bettercode/plugin", status: "ok", detail: "workspace package present" })
  } else {
    checks.push({ name: "@bettercode/plugin", status: "error", detail: "packages/bettercode-plugin/package.json not found" })
  }

  // Check 3: .bettercode/bettercode.jsonc
  const bcConfigPath = join(root, configDir, "bettercode.jsonc")
  if (existsSync(bcConfigPath)) {
    try {
      const raw = readFileSync(bcConfigPath, "utf8")
  ...
  // Check 4: .opencode/plugin/bettercode.ts
  const pluginFile = join(root, ".opencode", "plugin", "bettercode.ts")
  if (existsSync(pluginFile)) {
    const content = readFileSync(pluginFile, "utf8")
    if (content.includes("bettercode-plugin")) {
      checks.push({ name: "Plugin registration", status: "ok", detail: ".opencode/plugin/bettercode.ts present" })
    } else {
      checks.push({ name: "Plugin registration", status: "error", detail: ".opencode/plugin/bettercode.ts missing bettercode-plugin import" })
    }
  } else {
    checks.push({ name: "Plugin registration", status: "warn", detail: ".opencode/plugin/bettercode.ts not found (run bettercode sync)" })
  }
