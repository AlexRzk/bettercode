...
describe("bettercode sync", () => {
  it("creates .opencode/plugin/bettercode.js", () => {
  ...
  it("works with --json flag", () => {
    ...
    const parsed = JSON.parse(output)
    expect(parsed.status).toBe("ok")
    expect(parsed.pluginFile).toBe(".opencode/plugin/bettercode.js")
  })
})
...
describe("bettercode doctor", () => {
  it("reports ok for valid setup", () => {
    ...
    writeFileSync(join(fixtureDir, ".opencode", "plugin", "bettercode.js"), 'export default { id: "bettercode", server: () => ({}) }\n')
    writeFileSync(join(fixtureDir, ".opencode", "opencode.jsonc"), '{}\n')

    const output = runCli(["doctor"], fixtureDir)

    expect(output).toContain("BetterCode CLI")
    expect(output).toMatch(/(dist\/cli\.js present|src\/index\.ts present)/)
  ...
  it("warns when .opencode/plugin is missing", () => {
    ...
    expect(output).toContain("Plugin registration")
    expect(output).toContain("bettercode.js not found")
  })
...
describe("bettercode sync with pluginOptions", () => {
  it("writes plugin options to .opencode/opencode.jsonc", () => {
    ...
    expect(entry[0]).toBe("./plugin/bettercode.js")
  ...
  it("updates existing plugin entry without duplication", () => {
    ...
    mkdirSync(join(fixtureDir, ".opencode"), { recursive: true })
    writeFileSync(join(fixtureDir, ".opencode", "opencode.jsonc"), '{"plugin": [["./plugin/bettercode.js", {"brain": {"autoInject": false}}]]}\n')
    mkdirSync(join(fixtureDir, ".bettercode"), { recursive: true })
...
describe("bettercode run", () => {
  it("reports missing OpenCode CLI", () => {
