...
Practical reference for a future `packages/server` split after the bettercode
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/bettercode`.
...
Until enough shared service code lives outside `packages/bettercode`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/bettercode`

It should not import `packages/bettercode` services while `packages/bettercode`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on bettercode-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/bettercode` implementation details.
