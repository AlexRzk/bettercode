...
Trim `packages/bettercode/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/bettercode/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/bettercode`.
- `[x]` `bun typecheck` from `packages/bettercode`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/bettercode/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/bettercode`.
- `[x]` `bun dev generate > /tmp/bettercode-openapi.json` from `packages/bettercode`.
...
- Edit `packages/bettercode/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/bettercode-openapi.json` from `packages/bettercode`.
...
- Check source schemas in files like `packages/bettercode/src/session/schema.ts`, `packages/bettercode/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/bettercode/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/bettercode/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
