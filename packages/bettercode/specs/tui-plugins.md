...
- Author package entrypoint is `@bettercode/plugin/tui`.
...
```json
{
  "$schema": "https://bettercode.ai/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
  ...
  "plugin": ["@acme/bettercode-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@bettercode/plugin/tui`.
- `@bettercode/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@bettercode/plugin/tui"
...
- The value is a semver range checked against the running BetterCode version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.bettercode` is absent, no check is performed (backward compatible).
...
- `bettercode plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `bettercode plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.bettercode` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.bettercode`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.bettercode`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.bettercode` dir or `BETTERCODE_CONFIG_DIR`, BetterCode installs `@bettercode/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@bettercode/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus BetterCode's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
BetterCode registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@bettercode/plugin/tui`.
...
- `message` is required; `title` defaults to `"bettercode"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `bettercode.default`.
...
- Local plugins persist installed themes under the local `.bettercode/themes` area near the plugin config source.
...
- `--pure` / `BETTERCODE_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.bettercode/plugins/tui-smoke.tsx`
- Local vim plugin: `.bettercode/plugins/tui-vim.tsx`
- Local smoke config: `.bettercode/tui.json`
- Local smoke theme: `.bettercode/plugins/smoke-theme.json`
