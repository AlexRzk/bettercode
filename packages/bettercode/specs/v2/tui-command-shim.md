...
Delete `packages/bettercode/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/bettercode/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/bettercode/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/bettercode`
- TUI plugin loader tests in `packages/bettercode` if runtime plugin API wiring changed
