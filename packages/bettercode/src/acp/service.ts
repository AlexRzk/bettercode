...
import type { Message, BettercodeClient, SessionMessageResponse } from "@bettercode/sdk/v2"
...
export const AuthMethodID = "bettercode-login"
...
export class Service extends Context.Service<Service, Interface>()("@bettercode/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `bettercode auth login` in the terminal",
      name: "Login with bettercode",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: BettercodeClient) {
...
function makeUsageService(sdk: BettercodeClient) {
...
async function loadDirectorySnapshot(sdk: BettercodeClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, bettercode provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const bettercodeProvider = providers[ProviderV2.ID.make("bettercode")]
  const bettercodeModel = bettercodeProvider ? Provider.sort(Object.values(bettercodeProvider.models))[0] : undefined
  if (bettercodeProvider && bettercodeModel) return { providerID: bettercodeProvider.id, modelID: bettercodeModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "BetterCode service failure", service })
}
