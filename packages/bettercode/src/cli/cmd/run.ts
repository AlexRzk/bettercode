import type { PermissionV1 } from "@bettercode/core/v1/permission"
import { FSUtil } from "@bettercode/core/fs-util"
// CLI entry point for `bettercode run` and `bettercode --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`bettercode --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`bettercode --mini --attach`): connects to a running
//      bettercode server and runs interactive mode against it.
//
...
import { createBettercodeClient, type BettercodeClient, type ToolPart } from "@bettercode/sdk/v2"
import { FormatError, FormatUnknownError } from "../error"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

type ModelInput = Parameters<BettercodeClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run bettercode with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running bettercode server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to BETTERCODE_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to BETTERCODE_SERVER_USERNAME or 'bettercode')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: BettercodeClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: BettercodeClient, sessionID: string) {
      ...
      async function current(sdk: BettercodeClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: BettercodeClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: BettercodeClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: BettercodeClient) {
        ...
        async function loop(client: BettercodeClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createBettercodeClient({
        baseUrl: "http://bettercode.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
