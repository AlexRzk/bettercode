...
//
// Enable with BETTERCODE_DIRECT_TRACE=1. Writes one JSON line per event to
// ~/.local/share/bettercode/log/direct/<timestamp>-<pid>.jsonl. Also writes
...
export function trace(): Trace | undefined {
  ...
  if (!process.env.BETTERCODE_DIRECT_TRACE) {
    state = false
    return undefined
  }
