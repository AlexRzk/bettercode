...
const TUI_SCHEMA_URL = "https://bettercode.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from bettercode.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const bettercode = await bettercodeFiles(input)
  for (const file of bettercode) {
...
async function bettercodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "bettercode"),
    ...(await Filesystem.findUp(["bettercode.json", "bettercode.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "bettercode"))
  }
  if (Flag.BETTERCODE_CONFIG) files.push(Flag.BETTERCODE_CONFIG)
