...
import { isRecord } from "@bettercode/tui/util/record"
...
import { TuiKeybind } from "@bettercode/tui/config/keybind"
...
import { TuiConfig } from "@bettercode/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@bettercode/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old bettercode.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.bettercode`
  // folders between cwd and home, and BETTERCODE_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.BETTERCODE_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit BETTERCODE_TUI_CONFIG override, if set.
  if (Flag.BETTERCODE_TUI_CONFIG) {
    const configFile = Flag.BETTERCODE_TUI_CONFIG
  ...
  // 4. `.bettercode` directories (and BETTERCODE_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".bettercode") || dir === Flag.BETTERCODE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".bettercode") && dir !== Flag.BETTERCODE_CONFIG_DIR) continue
