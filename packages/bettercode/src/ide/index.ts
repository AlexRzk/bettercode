...
export function alreadyInstalled() {
  return process.env["BETTERCODE_CALLER"] === "vscode" || process.env["BETTERCODE_CALLER"] === "vscode-insiders"
}
...
export async function install(ide: (typeof SUPPORTED_IDES)[number]["name"]) {
  ...
  const p = await Process.run([cmd, "--install-extension", "sst-dev.bettercode"], {
    nothrow: true,
  })
