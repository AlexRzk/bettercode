...
export type PatchDeps = {
  ...
  files: (dir: string, name: "bettercode" | "tui") => string[]
}
...
function patchDir(input: PatchInput) {
  ...
  return path.join(root, ".bettercode")
}

function patchName(kind: Kind): "bettercode" | "tui" {
  if (kind === "server") return "bettercode"
  return "tui"
}
