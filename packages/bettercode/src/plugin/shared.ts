...
export const DEPRECATED_PLUGIN_PACKAGES = ["bettercode-openai-codex-auth", "bettercode-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, bettercodeVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(bettercodeVersion) || semver.major(bettercodeVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.bettercode
  if (typeof range !== "string") return
  if (!semver.satisfies(bettercodeVersion, range)) {
    throw new Error(`Plugin requires bettercode ${range} but running ${bettercodeVersion}`)
  }
}
