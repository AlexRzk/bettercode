...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@bettercode/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { errorData, errorMessage } from "@bettercode/tui/util/error"
...
import { hasTheme, upsertTheme } from "@bettercode/tui/context/theme"
import { Global } from "@bettercode/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@bettercode/core/util/flock"
import { Flag } from "@bettercode/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@bettercode/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@bettercode/core/v1/config/plugin"
import { createCommandShim } from "@bettercode/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@bettercode/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".bettercode"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".bettercode", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".bettercode", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.BETTERCODE_PURE ? [] : pluginOrigins
    if (Flag.BETTERCODE_PURE && pluginOrigins.length) {
    }
