...
import { BetterCodeHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for bettercode serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(BetterCodeHttpApi)
...
export const PublicApi = BetterCodeHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "bettercode",
    version: "1.0.0",
    description: "bettercode api",
