...
import type { LLMClientShape } from "@bettercode/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("bettercode"))
    return { type: "unsupported", reason: "provider is not openai, bettercode, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @bettercode/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains bettercode-owned. The native runtime only adapts
      // the @bettercode/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
