...
import { FrontmatterError } from "@bettercode/core/v1/config/error"
import { ConfigMarkdown } from "@/config/markdown"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Glob } from "@bettercode/core/util/glob"
...
const BETTERCODE_SKILL_PATTERN = "{skill,skills}/**/SKILL.md"
const SKILL_PATTERN = "**/SKILL.md"

// Built-in skill that ships with bettercode. The model's intuition for what an
// bettercode.json should look like is often wrong, and bettercode hard-fails on
// invalid config, so users hit cryptic startup errors. Loading this skill
// when the model is asked to touch bettercode's own config files gives it the
// actual schemas instead of guesses.
const CUSTOMIZE_BETTERCODE_SKILL_NAME = "customize-bettercode"
const CUSTOMIZE_BETTERCODE_SKILL_DESCRIPTION =
...
const CUSTOMIZE_BETTERCODE_SKILL_BODY = SkillPlugin.CustomizeOpencodeContent
...
const discoverSkills = Effect.fnUntraced(function* (
  ...
  for (const dir of configDirs) {
    yield* scan(state, dir, BETTERCODE_SKILL_PATTERN)
  }
...
export class Service extends Context.Service<Service, Interface>()("@bettercode/Skill") {}
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make(
      Effect.fn("Skill.state")(function* () {
        ...
        s.skills[CUSTOMIZE_BETTERCODE_SKILL_NAME] = {
          name: CUSTOMIZE_BETTERCODE_SKILL_NAME,
          description: CUSTOMIZE_BETTERCODE_SKILL_DESCRIPTION,
          location: "<built-in>",
          content: CUSTOMIZE_BETTERCODE_SKILL_BODY,
        }
