...
it.instance("plan agent denies edits except .bettercode/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".bettercode/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".bettercode", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.BETTERCODE_TEST_HOME
      process.env.BETTERCODE_TEST_HOME = test.directory
