...
describe("bettercode acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ bettercode }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ bettercode }))
        ...
        expect(initialized.agentInfo?.name).toBe("BetterCode")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ bettercode }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ bettercode })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("bettercode-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "bettercode-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.BETTERCODE_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ bettercode }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("bettercode-login")
