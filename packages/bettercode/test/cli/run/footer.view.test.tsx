...
import type { QuestionRequest } from "@bettercode/sdk/v2"
import { OpencodeKeymapProvider, registerOpencodeKeymap } from "@bettercode/tui/keymap"
import {
...
test("direct footer separates a lone context hint from model and command hint", async () => {
  const app = await renderFooter({
    providers: [provider()],
    currentModel: { providerID: "bettercode", modelID: "gpt-5" },
...
test("direct model panel renders current model selector", async () => {
  ...
  const [current] = createSignal<RunInput["model"]>({ providerID: "bettercode", modelID: "gpt-5" })
  ...
  try {
    ...
    expect(frame).toContain("Search")
    expect(frame).toContain("bettercode")
    expect(frame).toContain("GPT-5")
