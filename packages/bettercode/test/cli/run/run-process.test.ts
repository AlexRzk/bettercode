// Subprocess integration tests for `bettercode run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `bettercode.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `BETTERCODE_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("bettercode run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, bettercode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* bettercode.run("say hi")
        bettercode.expectExit(result, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, bettercode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* bettercode.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, bettercode }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* bettercode.run("think", { extraArgs: ["--thinking"] })
        bettercode.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* bettercode.run("think again")
        bettercode.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, bettercode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* bettercode.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, bettercode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* bettercode.run("say hi", { format: "json" })
        bettercode.expectExit(result, 0)

        const events = bettercode.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ bettercode }) =>
      Effect.gen(function* () {
        const result = yield* bettercode.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, bettercode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* bettercode.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, bettercode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* bettercode.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, bettercode }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* bettercode.run("request permission", { permission: { bash: "ask" } })
        bettercode.expectExit(denied, 0)
        ...
        const allowed = yield* bettercode.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        bettercode.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* bettercode.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        bettercode.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, bettercode }) =>
      Effect.gen(function* () {
        ...
        const server = yield* bettercode.serve()

        const result = yield* bettercode.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, bettercode }) =>
      Effect.gen(function* () {
        const result = yield* bettercode.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, bettercode }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* bettercode.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
