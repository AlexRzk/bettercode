...
import { BettercodeClient, type GlobalEvent } from "@bettercode/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<BettercodeClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<BettercodeClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<BettercodeClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<BettercodeClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<BettercodeClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://bettercode.test"),
...
function sdk(
  input: {
    ...
    subscribe?: BettercodeClient["event"]["subscribe"]
    globalEvent?: BettercodeClient["global"]["event"]
    promptAsync?: BettercodeClient["session"]["promptAsync"]
    status?: BettercodeClient["session"]["status"]
    messages?: BettercodeClient["session"]["messages"]
    children?: BettercodeClient["session"]["children"]
    permissions?: BettercodeClient["permission"]["list"]
    questions?: BettercodeClient["question"]["list"]
  } = {},
) {
  const client = new BettercodeClient()

  const subscribe: BettercodeClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: BettercodeClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: BettercodeClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: BettercodeClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: BettercodeClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: BettercodeClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: BettercodeClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: BettercodeClient["question"]["list"] = input.questions ?? (() => ok([]))
