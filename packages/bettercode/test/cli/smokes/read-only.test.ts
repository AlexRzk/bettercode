...
describe("bettercode read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // BETTERCODE_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ bettercode }) =>
      Effect.gen(function* () {
        const r = yield* bettercode.spawn(["mcp", "list"])
        bettercode.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ bettercode }) =>
      Effect.gen(function* () {
        const r = yield* bettercode.spawn(["providers", "list"])
        bettercode.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ bettercode }) =>
      Effect.gen(function* () {
        const r = yield* bettercode.spawn(["models"])
        bettercode.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ bettercode }) =>
      Effect.gen(function* () {
        const r = yield* bettercode.spawn(["agent", "list"])
        bettercode.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh BETTERCODE_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ bettercode }) =>
      Effect.gen(function* () {
        const r = yield* bettercode.spawn(["session", "list"])
        bettercode.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ bettercode }) =>
      Effect.gen(function* () {
        const r = yield* bettercode.spawn(["stats"])
        bettercode.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under BETTERCODE_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ bettercode }) =>
      Effect.gen(function* () {
        const r = yield* bettercode.spawn(["db", "path"])
        bettercode.expectExit(r, 0, "db path")
