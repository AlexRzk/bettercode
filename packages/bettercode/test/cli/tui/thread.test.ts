...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ bettercode }) =>
    Effect.gen(function* () {
      const result = yield* bettercode.spawn(["--replay-limit", "10"])

      bettercode.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ bettercode }) =>
    Effect.gen(function* () {
      const result = yield* bettercode.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ bettercode }) =>
    Effect.gen(function* () {
      const result = yield* bettercode.spawn(["--mini", "--port", "4096"])
