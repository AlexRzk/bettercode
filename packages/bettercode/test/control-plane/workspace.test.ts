...
import { EventSequenceTable } from "@bettercode/core/event/sql"
...
import { WorkspaceTable } from "@bettercode/core/control-plane/workspace.sql"
...
const originalEnv = {
  BETTERCODE_AUTH_CONTENT: process.env.BETTERCODE_AUTH_CONTENT,
  BETTERCODE_EXPERIMENTAL_WORKSPACES: process.env.BETTERCODE_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.BETTERCODE_EXPERIMENTAL_WORKSPACES = "true"
})
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@bettercode.test"`.cwd(dir).quiet()
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.BETTERCODE_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
        process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
        process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=bettercode-test"
        ...
        expect(JSON.parse(recorded.calls.create[0].env.BETTERCODE_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.BETTERCODE_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.BETTERCODE_EXPERIMENTAL_WORKSPACES).toBe("true")
        ...
        expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=bettercode-test")
