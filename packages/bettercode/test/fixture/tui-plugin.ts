...
import type { HostPluginApi } from "@bettercode/tui/plugin/slots"
...
export function createTuiPluginApi(opts: Opts = {}): HostPluginApi {
  ...
  const own = createBettercodeClient({
  ...
  let selected = opts.theme?.selected ?? "bettercode"
  const set =
    opts.theme?.set ??
    ((name: string) => {
  ...
  return {
    ...
    attention: {
      ...
      soundboard: {
        ...
        current: () => opts.attention?.soundboard?.current?.() ?? "bettercode.default",
