// Subprocess test harness for the bettercode CLI. Spawns the real binary against
...
//
// Configuration flows through bettercode's built-in test affordances:
//   - BETTERCODE_CONFIG_CONTENT      : provider config inline, no files to find
//   - BETTERCODE_TEST_HOME           : pins os.homedir() → tmpdir
//   - BETTERCODE_DISABLE_PROJECT_CONFIG : skip walking up for bettercode.json
//   - BETTERCODE_PURE                : skip external plugin discovery + install
//   - BETTERCODE_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `bettercode.run` is fully wired. The shape supports adding more
// builders (`bettercode.serve(opts)`, `bettercode.acp(opts)`, `bettercode.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of OpencodeCli.
import { test, type TestOptions } from "bun:test"
import { FSUtil } from "@bettercode/core/fs-util"
import { AppProcess } from "@bettercode/core/process"
...
const bettercodeRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(bettercodeRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    BETTERCODE_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    BETTERCODE_CONFIG_CONTENT: configJson,
    BETTERCODE_DISABLE_PROJECT_CONFIG: "1",
    BETTERCODE_PURE: "1",
    BETTERCODE_DISABLE_AUTOUPDATE: "1",
    BETTERCODE_DISABLE_AUTOCOMPACT: "1",
    BETTERCODE_DISABLE_MODELS_FETCH: "1",
    BETTERCODE_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `bettercode run`. New flags should
...
// `bettercode serve` is a long-lived process — it never exits on its own.
...
// `bettercode acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type OpencodeCli = {
  ...
  // Spawn `bettercode serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `bettercode acp` and return a duplex JSON-RPC handle. Long-lived:
...
export type CliFixture = {
  ...
  readonly bettercode: OpencodeCli
}
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("bettercode.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          BETTERCODE_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("bettercode.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("bettercode.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "bettercode server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `bettercode serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("bettercode.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const bettercode: OpencodeCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, bettercode })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `bettercode.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
...
function expectExit(result: RunResult, expected: number, label = "bettercode") {
...
// every call site. Use this for any test that needs the bettercode CLI fixture.
//
...
// resources (e.g. `bettercode.serve`) without an extra `Effect.scoped` wrapper —
