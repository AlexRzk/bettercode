    ...
    const msgs = [
      ...
      { role: "assistant", content: text("assistant string") },
      {
        ...
        content: [
          ...
          { type: "tool-call", toolCallId: "call-1", toolName: "Read", input: { filePath: ".bettercode/tool/emoji.ts" } },
          {
            type: "tool-result",
            toolCallId: "call-2",
            toolName: "Read",
...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    const bettercodeModel = {
      ...openaiModel,
      providerID: "bettercode",
      api: {
        id: "bettercode-test",
        url: "https://api.bettercode.ai",
        npm: "@ai-sdk/openai-compatible",
      },
    }
    const msgs = [
    ...
    const result = ProviderTransform.message(msgs, bettercodeModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.bettercode?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.bettercode?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    const msgs = [
      {
        ...
        providerOptions: {
          ...
          bettercode: { itemId: "msg_bettercode" },
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              bettercode: { itemId: "msg_bettercode_part" },
    ...
    expect(result[0].providerOptions?.bettercode?.itemId).toBe("msg_bettercode")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.bettercode?.itemId).toBe("msg_bettercode_part")
