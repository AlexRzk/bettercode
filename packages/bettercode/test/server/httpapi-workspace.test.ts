...
function requestServer(path: string, directory: string, init: RequestInit = {}) {
  ...
  headers.set("x-bettercode-directory", directory)
      ...
      try {
        const response = yield* requestDefault(url.toString(), dir, {
          ...
          headers: {
            "accept-encoding": "br",
            "content-type": "application/json",
            "x-bettercode-workspace": "internal",
          },
          body: JSON.stringify({ $schema: "https://bettercode.ai/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-bettercode-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-bettercode-workspace")
