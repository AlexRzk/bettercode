...
import { SessionV1 } from "@bettercode/core/v1/session"
import { FSUtil } from "@bettercode/core/fs-util"
import { ModelsDev } from "@bettercode/core/models-dev"
import { HttpRecorder } from "@bettercode/http-recorder"
import { HttpRecorderInternal } from "@bettercode/http-recorder/internal"
...
import { LLMEvent, LLMResponse } from "@bettercode/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor } from "@bettercode/llm/route"
import { Env } from "@/env"
...
const zenURL = (connection: string) => `https://console.bettercode.ai/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.BETTERCODE_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["bettercode", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("BETTERCODE_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("BETTERCODE_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["bettercode", "native", "oauth", "tool-loop"],
  ...
  {
    id: "bettercode-proxy",
    name: "BetterCode proxy",
    providerID: ProviderV2.ID.bettercode,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["bettercode", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.BETTERCODE_RECORD_CONSOLE_TOKEN && process.env.BETTERCODE_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.bettercode,
        name: "BetterCode Zen",
        env: ["BETTERCODE_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.BETTERCODE_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.BETTERCODE_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.BETTERCODE_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["bettercode", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("BETTERCODE_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("BETTERCODE_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("BETTERCODE_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set BETTERCODE_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the bettercode LLM stack remain real.
  const metadata = {
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "bettercode.json"),
      JSON.stringify({ $schema: "https://bettercode.ai/config.json", ...scenario.config(model) }),
    ),
  )
