...
import { LLMEvent, ToolFailure } from "@bettercode/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor, type LLMClientShape } from "@bettercode/llm/route"
...
describe("session.llm-native.request", () => {
  ...
  test("selects native request routes for provider packages", () => {
    ...
    const compatible = LLMNative.model({
      model: {
        ...baseModel,
        providerID: ProviderV2.ID.make("bettercode"),
  ...
  test("only enables native runtime for supported OpenAI API-key models", () => {
    ...
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("bettercode") },
        provider: { ...providerInfo, id: ProviderV2.ID.make("bettercode") },
        auth: undefined,
      }),
    ).toMatchObject({
      type: "supported",
    ...
    ).toEqual({ type: "unsupported", reason: "provider is not openai, bettercode, or anthropic" })
    expect(
      LLMNativeRuntime.status({
        model: baseModel,
  ...
  test("prefers console provider api key over stored bettercode auth", () => {
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("bettercode") },
        provider: {
          ...providerInfo,
          id: ProviderV2.ID.make("bettercode"),
