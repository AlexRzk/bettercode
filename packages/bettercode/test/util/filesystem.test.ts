...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "bettercode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "bettercode.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "bettercode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "bettercode.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["bettercode.json", "bettercode.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "bettercode.json"),
        path.join(project, "bettercode.jsonc"),
      ])
    })
  })
