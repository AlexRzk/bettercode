...
for (const item of targets) {
  ...
  const result = await Bun.build({
    ...
    define: {
      ...
      BETTERCODE_CLI_NAME: `'${binary}'`,
      BETTERCODE_MODELS_DEV: modelsData,
      BETTERCODE_CHANNEL: `'${Script.channel}'`,
      BETTERCODE_LIBC: item.os === "linux" ? `'${item.abi ?? "glibc"}'` : "undefined",
  ...
  await Bun.write(
    ...
    JSON.stringify(
      {
        name: `@bettercode/${name}`,
        version: Script.version,
        license: "MIT",
        repository: { type: "git", url: "git+https://github.com/anomalyco/bettercode.git" },
        os: [item.os],
        cpu: [item.arch],
      },
      null,
