...
export function createWslServersController(
  ...
  const checkOpencode = async (distro: string, opts?: { signal?: AbortSignal }) => {
    ...
    const version = resolved
      ...
      : null
    return bettercodeCheck(distro, resolved, version, appVersion)
  }
  ...
  const refreshOpencodeCheckBackground = (id: string, distro: string) => {
    void checkOpencode(distro)
      ...
      .catch((error) => {
        ...
        logger?.error("wsl bettercode check failed", { id, distro, message })
      })
  }
    ...
    await Promise.all(
      state.servers.map((item) =>
        checkOpencode(item.config.distro)
          ...
          .catch((error) => {
            const message = error instanceof Error ? error.message : String(error)
            logger?.error("wsl bettercode check failed", {
              id: item.config.id,
  ...
  return {
    ...
    async probeOpencode(name: string) {
      await runJob({ kind: "probe-bettercode", distro: name, startedAt: Date.now() }, async (abort) => {
        await refreshOpencodeCheck(name, { signal: abort.signal })
      })
    },

    async installOpencode(name: string) {
      await runJob({ kind: "install-bettercode", distro: name, startedAt: Date.now() }, async (abort) => {
        const result = await installWslOpencode(appVersion, name, { signal: abort.signal })
        if (result.code !== 0) {
          throw new Error(summarize(result.stderr || result.stdout) || "BetterCode installation failed")
        }
        await refreshOpencodeCheck(name, { signal: abort.signal })
        expectOpencodeVersion(state.bettercodeChecks[name]?.version ?? null, appVersion, name)
    ...
    async removeServer(id: string) {
      ...
      setState({
        ...
        ...(distro ? clearWslDistroState(state.distroProbes, state.bettercodeChecks, distro) : {}),
      })
    },
...
function bettercodeCheck(
  ...
  if (!resolvedPath) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "bettercode is not installed in this distro",
    }
  }
  if (!version) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "bettercode is installed but could not run",
    }
  }
  return {
    distro,
