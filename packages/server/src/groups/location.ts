...
function ref(request: HttpServerRequest.HttpServerRequest): Location.Ref {
  ...
  const workspaceID = query.get("location[workspace]") || request.headers["x-bettercode-workspace"]
  const directory =
    query.get("location[directory]") ||
    (request.headers["x-bettercode-directory"] ? decode(request.headers["x-bettercode-directory"]) : process.cwd())
