...
import { ProviderIcon } from "@bettercode/ui/provider-icon"
...
import {
  ...
  type StatsModelData,
  type UsageRange,
} from "@bettercode/stats-core/domain/home"
...
const statsCanonicalBaseUrl = "https://bettercode.ai/data/"
const statsUnfurlPath = "banner.png"
const statsUnfurlAlt = "BetterCode Data wordmark on a dark patterned background"
...
export default function StatsModel() {
  ...
  const modelTitle = createMemo(() => `${modelName()} Usage, Cost & Rank | BetterCode Data`)
  const modelDescription = createMemo(
    () =>
      `View ${modelName()} BetterCode Go usage data, including token volume, weekly rank, token mix, costs, cache ratio, sessions, geo breakdowns, and peer models.`,
  )
  ...
  return (
    <main data-page="stats" data-theme={themePreference()}>
      ...
      <Meta property="og:site_name" content="BetterCode" />
      ...
      <div data-slot="model-hero-grid">
        <div data-slot="model-hero-copy">
          ...
          <Show
            when={props.data}
            fallback={
              <p>Model facts from the shared model index. BetterCode Go usage appears once this model has activity.</p>
            }
          >
            {(data) => (
              <p>
                {data().rank === null
                  ? "Unranked across last week's BetterCode Go usage"
                  : `Ranked #${data().rank} across last week's BetterCode Go usage`}{" "}
...
function ModelCatalogCallout(props: { catalog: ModelCatalogEntry | null }) {
  return (
    <div data-component="model-rank-panel">
      ...
      <p>No BetterCode Go usage in the current data window.</p>
    </div>
  )
}
...
function ModelOverview(props: { data: StatsModelData | null }) {
  return (
    <section data-section="model-panel">
      <SectionTitle title="Overview" description="Recent BetterCode Go tokens, unique users, and market position." />
      <Show
        when={props.data}
        fallback={
          <ModelEmptyState title="No usage summary" description="This model has no BetterCode Go usage rows yet." />
        }
      >
        {(data) => (
...
function ModelUsageSection(props: { data: ModelUsagePoint[] }) {
  return (
    <section id="usage" data-section="model-panel">
      <SectionTitle title="Usage" description="Daily BetterCode Go token volume over the recent two-month window." />
      <Show
...
function ModelUsersSection(props: { data: ModelUsagePoint[] }) {
  return (
    <section id="users" data-section="model-panel">
      <SectionTitle
        title="Unique Users"
        description="Daily unique BetterCode Go users over the recent two-month window."
      />
      <Show
...
function ModelEfficiencySection(props: { data: StatsModelData | null; catalog: ModelCatalogEntry | null }) {
  return (
    <section id="efficiency" data-section="model-panel">
      <SectionTitle title="Efficiency" description="Cost, cache behavior, and average BetterCode Go session shape." />
      <Show
        when={props.data}
        fallback={
...
function ModelGeoBreakdownSection(props: { data: Record<UsageRange, CountryEntry[]> }) {
  ...
  return (
    ...
    >
      <SectionTitle title="Geo Breakdown" description="BetterCode Go model tokens used by country." />
      <Show
        when={data().length > 0}
        fallback={
          <ModelEmptyState title="No geo data" description="No BetterCode Go geo_stat rows matched this model." />
        }
      >
...
function ModelPeersSection(props: { data: StatsModelData | null }) {
  return (
    <section id="peers" data-section="model-panel">
      <SectionTitle title="Peers" description="Nearby models by recent BetterCode Go token volume." />
      <Show
