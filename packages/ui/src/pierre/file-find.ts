...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("bettercode-find")
  api.delete("bettercode-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("bettercode-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("bettercode-find", new Highlight(...rest))
    return true
  }
