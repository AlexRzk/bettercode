...
أنشئ الملف `.bettercode/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات BetterCode أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ BetterCode:
...
- على مستوى النظام: `~/.config/bettercode/commands/`
- لكل مشروع: `.bettercode/commands/`
...
يتضمن bettercode عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
