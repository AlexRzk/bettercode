...
description: أنشئ أدوات يمكن لـ LLM استدعاؤها في bettercode.
---

الأدوات المخصصة هي دوال تنشئها يمكن لـ LLM استدعاؤها أثناء المحادثات. تعمل جنبا إلى جنب مع [الأدوات المدمجة](/docs/tools) في bettercode مثل `read` و`write` و`bash`.
...
- محليا بوضعها في مجلد `.bettercode/tools/` داخل مشروعك.
- أو عالميا بوضعها في `~/.config/bettercode/tools/`.
