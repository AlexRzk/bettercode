---
title: المزوّدون
description: استخدام أي موفّر LLM في BetterCode.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم BetterCode ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات BetterCode.
...
في `~/.local/share/bettercode/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات BetterCode.
...
BetterCode Zen هي قائمة نماذج يوفّرها فريق BetterCode وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع BetterCode. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع BetterCode Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر bettercode، ثم انتقل إلى [bettercode.ai/auth](https://bettercode.ai/auth).
...
يعمل مثل أي مزوّد آخر في BetterCode واستخدامه اختياري بالكامل.

---

## BetterCode Go

BetterCode Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق BetterCode، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع BetterCode.

1. شغّل الأمر `/connect` في TUI، واختر `BetterCode Go`، ثم انتقل إلى [bettercode.ai/auth](https://bettercode.ai/zen).
...
لاستخدام Amazon Bedrock مع BetterCode:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل bettercode:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `bettercode.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل BetterCode غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين bettercode لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل bettercode بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات bettercode.
...
تخزين الرمز في مساحة تخزين مصادقة bettercode.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم BetterCode نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر BetterCode
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`bettercode.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
لاستخدام اشتراك GitHub Copilot مع bettercode:
...
لاستخدام Google Vertex AI مع BetterCode:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر bettercode، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [bettercode-helicone-session](https://github.com/H2Shami/bettercode-helicone-session) لتسجيل كل محادثة BetterCode تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة BetterCode مدرجة كجلسة مستقلة.
...
يمكنك ضبط bettercode لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط bettercode لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط bettercode لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع BetterCode. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/bettercode) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع BetterCode:
...
3. انسخ مفتاح API لاستخدامه في BetterCode.
...
6. **مهم**: قبل استخدام نماذج السحابة في BetterCode، يجب سحب معلومات النموذج محليا:
...
BetterCode Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق BetterCode. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>BetterCode Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **BetterCode Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ BetterCode.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات bettercode.
...
5. يمكنك أيضا تخصيصها عبر إعدادات bettercode. إليك مثالا على تحديد مزوّد
...
يمكنك أيضا تخصيص النماذج عبر إعدادات bettercode. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع bettercode. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `bettercode.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ BetterCode بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `bettercode auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات bettercode وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات bettercode.
