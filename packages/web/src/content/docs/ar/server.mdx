---
title: الخادم
description: تفاعل مع خادم bettercode عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `bettercode serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل bettercode استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `bettercode.local` |
...
عيّن `BETTERCODE_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `bettercode`، أو عيّن `BETTERCODE_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `bettercode serve` و `bettercode web`.
...
عند تشغيل `bettercode` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم bettercode للتفاعل مع bettercode برمجيا.
:::

تتيح هذه البنية لـ bettercode دعم عدة عملاء وتمكّنك من التفاعل مع bettercode برمجيا.

يمكنك تشغيل `bettercode serve` لبدء خادم مستقل. إذا كانت واجهة bettercode في terminal (TUI)
قيد التشغيل، فسيبدأ `bettercode serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات BetterCode لـ [IDE](/docs/ide).
...
يعرض خادم bettercode واجهات API التالية.
