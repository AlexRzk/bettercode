...
تتيح مهارات الوكيل لـ BetterCode اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث BetterCode في هذه المواقع:

- إعدادات المشروع: `.bettercode/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/bettercode/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج BetterCode صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.bettercode/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/bettercode/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.bettercode/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض BetterCode المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `bettercode.json`:
...
**للوكلاء المدمجين** (في `bettercode.json`):
