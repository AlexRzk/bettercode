...
مع BetterCode يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم BetterCode سمتنا `bettercode`.
...
يأتي BetterCode مع عدة سمات مدمجة.
...
- يريدون أن يطابق BetterCode مظهر terminal لديهم
...
يدعم BetterCode نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/bettercode/themes/*.json` أو `$XDG_CONFIG_HOME/bettercode/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.bettercode/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.bettercode/themes/*.json`
