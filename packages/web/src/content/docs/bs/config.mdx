---
title: Konfiguracija
description: Korištenje BetterCode JSON konfiguracije.
---

Možete konfigurirati BetterCode koristeći JSON konfiguracijski fajl.

---

## Format

BetterCode podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/bettercode`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/bettercode/bettercode.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`BETTERCODE_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`bettercode.json` u projektu) - postavke specifične za projekat
5. **`.bettercode` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`BETTERCODE_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.bettercode` i `~/.config/bettercode` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/bettercode` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu BetterCode konfiguraciju u `~/.config/bettercode/bettercode.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/bettercode/tui.json`.
...
Dodajte `bettercode.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se BetterCode pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `BETTERCODE_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `BETTERCODE_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.bettercode` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.bettercode`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`bettercode.ai/config.json`**](https://bettercode.ai/config.json).

TUI konfiguracija koristi [**`bettercode.ai/tui.json`**](https://bettercode.ai/tui.json).
...
Koristite `BETTERCODE_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `bettercode.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `bettercode serve` i `bettercode web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš BetterCode server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `bettercode.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj BetterCode konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, BetterCode pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/bettercode/agents/` ili `.bettercode/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, BetterCode će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`bettercode run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/bettercode/commands/` ili `.bettercode/commands/`. [Saznajte više ovdje](/docs/commands).
...
BetterCode će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, BetterCode **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju BetterCode sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.bettercode/plugins/` ili `~/.config/bettercode/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite BetterCode da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
