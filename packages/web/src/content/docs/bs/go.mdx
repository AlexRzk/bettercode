...
BetterCode Go je povoljna pretplata — **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno** — koja vam pruža pouzdan pristup popularnim otvorenim modelima za programiranje.

Go radi kao bilo koji drugi provajder u BetterCode-u. Pretplatite se na BetterCode Go i
dobijete svoj API ključ. On je **potpuno opcionalan** i ne morate ga koristiti da
biste koristili BetterCode.
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa BetterCode-om.
:::
...
BetterCode Go vam daje pristup ovim modelima za **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno**.

---

## Kako funkcioniše

BetterCode Go radi kao bilo koji drugi provajder u BetterCode-u.

1. Prijavite se na **<a href={console}>BetterCode Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite komandu `/connect` u TUI-ju, odaberite `BetterCode Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih kroz Go.

:::note
Samo jedan član po radnom prostoru (workspace) može se pretplatiti na BetterCode Go.
:::
...
BetterCode Go uključuje sljedeća ograničenja:
...
[Model id](/docs/config/#models) u vašoj BetterCode konfiguraciji
koristi format `bettercode-go/<model-id>`. Na primjer, za Kimi K2.7 Code, koristili biste
`bettercode-go/kimi-k2.7-code` u svojoj konfiguraciji.
...
Napravili smo BetterCode Go da bismo:
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz BetterCode koristite i bilo kojeg drugog provajdera.
