---
title: LSP serveri
description: BetterCode se integriše sa vašim LSP serverima.
---

BetterCode se može integrisati sa Language Server Protocol (LSP) serverima kako bi koristio dijagnostiku kao feedback za agenta.

## Ugrađeni

BetterCode dolazi sa nekoliko ugrađenih LSP servera za popularne jezike:
...
:::note
Možete onemogućiti automatska preuzimanja LSP servera tako što ćete postaviti varijablu okruženja `BETTERCODE_DISABLE_LSP_DOWNLOAD` na `true`.
:::
...
Kada je LSP omogućen i bettercode otvori fajl, on:
...
Možete omogućiti i prilagoditi LSP servere kroz `lsp` odjeljak u vašoj bettercode konfiguraciji.
