...
Možete dodati vanjske alate u BetterCode koristeći _Model Context Protocol_, ili MCP. BetterCode podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [BetterCode Config](https://bettercode.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/bettercode`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
BetterCode automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, BetterCode će:
...
Ako server zahtijeva autentifikaciju, BetterCode će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `bettercode mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, BetterCode sigurno čuva tokene u `~/.local/share/bettercode/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u BetterCode, zajedno s ugrađenim alatima. Možete njima upravljati kroz BetterCode konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete BetterCode sa vašim Sentry nalogom.
