---
title: Dodaci
description: Napišite vlastite dodatke za proširenje BetterCode.
---

Dodaci vam omogućavaju da proširite BetterCode spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja BetterCode.
...
- `.bettercode/plugins/` - Dodaci na nivou projekta
- `~/.config/bettercode/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/bettercode/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/bettercode/bettercode.json`)
2. Konfiguracija projekta (`bettercode.json`)
3. Globalni direktorij dodataka (`~/.config/bettercode/plugins/`)
4. Direktorij dodataka projekta (`.bettercode/plugins/`)
...
BetterCode pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje BetterCode.
...
Spriječite bettercode da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u bettercode:
...
Pomoćnik `tool` kreira prilagođeni alat koji bettercode može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za bettercode zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://bettercode.ai/docs/sdk) za detalje.
