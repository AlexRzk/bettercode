---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u BetterCode.
---

import config from "../../../../config.mjs"
export const console = config.console

BetterCode koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj BetterCode konfiguraciji.
...
u `~/.local/share/bettercode/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem BetterCode
config.
...
BetterCode Zen je lista modela koje je obezbedio BetterCode tim koji su bili
testirano i potvrđeno da dobro radi sa BetterCode. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa BetterCode Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite bettercode i idite na [bettercode.ai/auth](https://bettercode.ai/auth).
...
Radi kao i svaki drugi provajder u BetterCode i potpuno je opcionalan za korištenje.

---

## BetterCode Go

BetterCode Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža BetterCode tim i koji su testirani i verificirani da dobro rade s BetterCode-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `BetterCode Go` i idite na [bettercode.ai/auth](https://bettercode.ai/zen).
...
Da biste koristili Amazon Bedrock s BetterCode:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete bettercode:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `bettercode.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u BetterCode.
:::
...
Možete konfigurirati bettercode za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi bettercode ispravno radio.
   :::
...
Također možete dodati modele kroz svoju bettercode konfiguraciju.
...
da pohrani token u bettercode auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
BetterCode koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate BetterCode
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`bettercode.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Prilagodite putem `bettercode.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s bettercode:
...
Za korištenje Google Vertex AI s BetterCode:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće bettercode.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem bettercodea, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [bettercode-helicone-session](https://github.com/H2Shami/bettercode-helicone-session) da automatski prijavite svaki BetterCode razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki BetterCode razgovor naveden kao zasebna sesija.
...
Možete konfigurirati bettercode za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati bettercode za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati bettercode za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za BetterCode. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/bettercode) za detalje.
:::
...
Da biste koristili Ollama Cloud s BetterCode:
...
3. Kopirajte API ključ za korištenje u BetterCode.
...
6. **Važno**: Prije upotrebe modela oblaka u BetterCode, morate lokalno povući informacije o modelu:
...
BetterCode Zen je lista testiranih i verifikovanih modela koju je obezbedio BetterCode tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>BetterCode Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **BetterCode Zen**.

```txt
   /connect
```

3. Unesite svoj BetterCode API ključ.
...
Također možete dodati dodatne modele putem vaše bettercode konfiguracije.
...
5. Također ih možete prilagoditi putem vaše bettercode konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju bettercode konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s bettercode-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `bettercode.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju BetterCode da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `bettercode auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite BetterCode konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj bettercode konfiguraciji.
