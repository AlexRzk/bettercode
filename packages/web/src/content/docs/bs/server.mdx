---
title: Server
description: Komunicirajte s bettercode serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `bettercode serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji bettercode klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `bettercode.local` |
...
Postavite `BETTERCODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `bettercode`, ili postavite `BETTERCODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `bettercode serve` i za `bettercode web`.
...
Kada pokrenete `bettercode`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite bettercode server za programsku interakciju sa bettercode-om.
:::

Ova arhitektura omogucava bettercode podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `bettercode serve` da startate standalone server. Ako je bettercode TUI vec pokrenut, `bettercode serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste BetterCode [IDE](/docs/ide) pluginovi.
...
bettercode server izlaže sljedece API-je.
