...
Agent skills omogucavaju BetterCode da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
BetterCode pretrazuje ove lokacije:

- Konfiguracija projekta: `.bettercode/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/bettercode/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, BetterCode ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.bettercode/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/bettercode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.bettercode/skills/git-release/SKILL.md` ovako:
...
BetterCode navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `bettercode.json`:
...
**Za ugradene agente** (u `bettercode.json`):
