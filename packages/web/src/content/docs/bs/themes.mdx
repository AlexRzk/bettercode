...
U BetterCode mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, BetterCode koristi nasu `bettercode` temu.
...
BetterCode dolazi sa vise ugradenih tema.
...
- Zele da BetterCode odgovara izgledu njihovog terminala
...
BetterCode podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/bettercode/themes/*.json` ili `$XDG_CONFIG_HOME/bettercode/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.bettercode/themes/*.json`
4. **Trenutni radni direktorij** - `./.bettercode/themes/*.json`
