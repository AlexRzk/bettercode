...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. BetterCode dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u BetterCode.
...
:::note
Ovaj alat je dostupan samo kada je `BETTERCODE_EXPERIMENTAL_LSP_TOOL=true` (ili `BETTERCODE_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz BetterCode provajdera ili kada je varijabla `BETTERCODE_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju BetterCode:
