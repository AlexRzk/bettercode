...
Da biste otklonili probleme s BetterCode, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\bettercode\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `bettercode --log-level DEBUG`.

---

## Pohrana

BetterCode pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/bettercode/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\bettercode`
...
BetterCode Desktop pokreće lokalni BetterCode server (`bettercode-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `BetterCode` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/bettercode/bettercode.jsonc` (ili `~/.config/bettercode/bettercode.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/bettercode/bettercode.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\bettercode\bettercode.jsonc`
...
BetterCode također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/bettercode/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\bettercode\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga BetterCode mogao ponovo izgraditi.

1. Potpuno zatvorite BetterCode Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/bettercode`
- **Linux**: obrišite `~/.cache/bettercode` (ili pokrenite `rm -rf ~/.cache/bettercode`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\bettercode`

3. Ponovo pokrenite BetterCode Desktop.

---

### Rješavanje problema sa vezom na serveru

BetterCode Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `bettercode.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `BETTERCODE_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `BETTERCODE_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, BetterCode Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa BetterCode karakteristikama.

---

### Obavještenja se ne prikazuju

BetterCode Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za BetterCode u postavkama vašeg OS-a, i
...
1. Zatvorite BetterCode Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka BetterCode Desktop aplikacije):

- `bettercode.settings.dat` (URL zadanog servera za desktop)
- `bettercode.global.dat` i `bettercode.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Ako imate problema s BetterCode:
...
Da saznate kojim modelima imate pristup, pokrenite `bettercode models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\bettercode`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. BetterCode dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\bettercode`

2. Ponovo pokrenite BetterCode da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti BetterCode da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
BetterCode će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
