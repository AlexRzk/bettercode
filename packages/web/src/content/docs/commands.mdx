...
Create `.bettercode/commands/test.md`:
...
You can add custom commands through the BetterCode config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your BetterCode [config](/docs/config):
...
- Per-project: `.bettercode/commands/`
...
bettercode includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
