---
title: Plugins
description: Skriv dine egne plugins for at udvide BetterCode.
---

Plugins giver dig mulighed for at udvide BetterCode ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre BetterCodes standardadfærd.
...
- `.bettercode/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/bettercode/node_modules/`.
...
1. Global konfiguration (`~/.config/bettercode/bettercode.json`)
2. Projektkonfiguration (`bettercode.json`)
3. Global plugin-mappe (`~/.config/bettercode/plugins/`)
4. Projekt plugin bibliotek (`.bettercode/plugins/`)
...
BetterCode kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En bettercode SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide bettercode.
...
:::note
Hvis du bruger BetterCode desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå bettercode i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til bettercode:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som bettercode kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for bettercode sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://bettercode.ai/docs/sdk) for detaljer.
