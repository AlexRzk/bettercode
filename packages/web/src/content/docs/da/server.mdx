---
title: Server
description: Interager med bettercode-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `bettercode serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en bettercode-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `bettercode.local` |
...
Indstil `BETTERCODE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `bettercode`, eller indstil `BETTERCODE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `bettercode serve` og `bettercode web`.
...
Når du kører `bettercode` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug bettercode-serveren til at interagere med bettercode programmatisk.
:::

Denne arkitektur lader bettercode understøtte flere klienter og lader dig interagere med bettercode programmatisk.

Du kan køre `bettercode serve` for at starte en selvstændig server. Hvis du har
bettercode TUI kørende, vil `bettercode serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af BetterCode [IDE](/docs/ide) plugins.
...
BetterCode-serveren viser følgende API'er.
