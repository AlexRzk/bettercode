...
Med BetterCode kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger BetterCode vårt eget `bettercode`-tema.
...
BetterCode støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/bettercode/themes/*.json` eller `$XDG_CONFIG_HOME/bettercode/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.bettercode/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.bettercode/themes/*.json`
