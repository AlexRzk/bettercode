...
Her er alle de innebygde verktøyene som er tilgængelige i BetterCode.
...
:::note
Dette verktøyet er bare tilgængelig når `BETTERCODE_EXPERIMENTAL_LSP_TOOL=true` (eller `BETTERCODE_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger BetterCode-leverandøren eller når miljøvariabelen `BETTERCODE_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
