...
Es gibt zwei Arten von Agenten in BetterCode; Primäragenten und Subagenten.
...
BetterCode verfügt über zwei integrierte Primäragenten: **Build** und **Plan**. Auch
...
Konfigurieren Sie Agenten in Ihrer `bettercode.json`-Konfigurationsdatei:
...
Wenn keine Temperatur angegeben ist, verwendet BetterCode modellspezifische Standardwerte; typischerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle.
...
Dieser Pfad ist relativ zum Speicherort der Konfigurationsdatei. Dies funktioniert sowohl für die globale BetterCode-Konfiguration als auch für die projektspezifische Konfiguration.
...
Die Modell-ID in Ihrer BetterCode-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [BetterCode Zen](/docs/zen) verwenden, würden Sie `bettercode/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
...
:::tip
Führen Sie `bettercode models` aus, um eine Liste der verfügbaren Modelle anzubieten.
:::
...
:::tip
Haben Sie einen Agenten, den Sie gerne teilen möchten? [Reichen Sie einen PR ein](https://github.com/anomalyco/bettercode).
:::
