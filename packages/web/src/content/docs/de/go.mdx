...
BetterCode Go ist ein kostengünstiges Abonnement — **5 $ für deinen ersten Monat**, danach **10 $/Monat** —, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

Go funktioniert wie jeder andere Provider in BetterCode. Du abonnierst BetterCode Go und
erhältst deinen API-Key. Es ist **völlig optional** und du musst es nicht nutzen, um
BetterCode zu verwenden.
...
BetterCode Go bietet dir Zugriff auf diese Modelle für **5 $ im ersten Monat**, danach **10 $/Monat**.

---

## Wie es funktioniert

BetterCode Go funktioniert wie jeder andere Provider in BetterCode.

1. Du meldest dich bei **<a href={console}>BetterCode Zen</a>** an, abonnierst Go und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `BetterCode Go` und fügst deinen API-Key ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann BetterCode Go abonnieren.
:::
...
BetterCode Go beinhaltet die folgenden Limits:
...
Die [Modell-ID](/docs/config/#models) in deiner BetterCode Config verwendet das Format `bettercode-go/<model-id>`. Für Kimi K2.7 Code würdest du beispielsweise `bettercode-go/kimi-k2.7-code` in deiner Config verwenden.
...
Wir haben BetterCode Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit BetterCode zu nutzen.
