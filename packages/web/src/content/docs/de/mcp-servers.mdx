...
Sie können externe Tools zu BetterCode hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. BetterCode unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [BetterCode Config](https://bettercode.ai/docs/config/) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.
...
Organisationen können über ihren `.well-known/bettercode`-Endpunkt Standard-MCP-Server bereitstellen. Diese Server sind möglicherweise standardmäßig deaktiviert, sodass Benutzer sich für den Server entscheiden können, die sie benötigen.
...
BetterCode übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird BetterCode Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von BetterCode beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `bettercode mcp auth <server-name>` verwenden.
...
Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert BetterCode die Token sicher in `~/.local/share/bettercode/mcp-auth.json`.
...
Ihre MCPs sind neben integrierten Tools auch als Tools in BetterCode verfügbar. Sie können sie also wie jedes andere Tool über die BetterCode-Konfiguration verwalten.
...
Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und BetterCode mit Ihrem Sentry-Konto zu verbinden.
