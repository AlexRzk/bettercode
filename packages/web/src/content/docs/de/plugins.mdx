---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um BetterCode zu erweitern.
---

Mit Plugins können Sie BetterCode erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von BetterCode zu ändern.
...
- `.bettercode/plugins/` – Plugins auf Projektebene
- `~/.config/bettercode/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/bettercode/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/bettercode/bettercode.json`)
2. Projektkonfiguration (`bettercode.json`)
3. Globales Plugin-Verzeichnis (`~/.config/bettercode/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.bettercode/plugins/`)
...
BetterCode führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein BetterCode-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie BetterCode erweitern können.
...
:::note
Wenn Sie die BetterCode-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass BetterCode `.env`-Dateien liest:
...
Plugins können BetterCode auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das BetterCode aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für BetterCode zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://bettercode.ai/docs/sdk).
