---
title: Regeln
description: Lege eigene Anweisungen fuer bettercode fest.
---

Du kannst bettercode ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in bettercode den Befehl `/init` aus.
...
So kann bettercode sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht bettercode sie zu erweitern.
...
bettercode kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/bettercode/AGENTS.md` speichern.
Diese gelten in allen bettercode-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt BetterCode dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/bettercode/AGENTS.md` existiert)
...
Beim Start sucht bettercode in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/bettercode/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `bettercode.json` oder in `~/.config/bettercode/bettercode.json` angeben.
...
bettercode parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit bettercode.json

Empfohlen ist das `instructions`-Feld in der `bettercode.json`:
...
Du kannst bettercode explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `bettercode.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
