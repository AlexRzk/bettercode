---
title: Server
description: Nutze den bettercode-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `bettercode serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein bettercode-Client nutzen kann.
...
Setze `BETTERCODE_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `bettercode`, kann aber mit `BETTERCODE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `bettercode serve` und `bettercode web`.
...
Wenn du `bettercode` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den bettercode-Server, um bettercode programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `bettercode serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `bettercode serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die BetterCode-[IDE](/docs/ide)-Plugins.
...
Der bettercode-Server stellt folgende APIs bereit.
