...
Agent Skills erlauben BetterCode, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
BetterCode sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft BetterCode vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.bettercode/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/bettercode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.bettercode/skills/git-release/SKILL.md` zum Beispiel so:
...
BetterCode listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `bettercode.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `bettercode.json`):
