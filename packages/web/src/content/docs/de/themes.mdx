...
Mit BetterCode kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet BetterCode das `bettercode`-Theme.
...
BetterCode liefert mehrere eingebaute Themes mit.
...
- BetterCode optisch ans Terminal anpassen wollen
...
BetterCode unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/bettercode/themes/*.json` oder `$XDG_CONFIG_HOME/bettercode/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.bettercode/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.bettercode/themes/*.json`
