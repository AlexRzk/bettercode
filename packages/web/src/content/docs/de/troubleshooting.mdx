...
Wenn BetterCode Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\bettercode\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `bettercode --log-level DEBUG`.

---

## Speicher

bettercode speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/bettercode/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\bettercode`
...
BetterCode Desktop startet im Hintergrund einen lokalen BetterCode-Server (`bettercode-cli`-Sidecar).
...
- Nur macOS: `BetterCode`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/bettercode/bettercode.jsonc` (oder `~/.config/bettercode/bettercode.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/bettercode/bettercode.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\bettercode\bettercode.jsonc`
...
BetterCode kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/bettercode/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\bettercode\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit BetterCode ihn neu aufbauen kann.

1. Beende BetterCode Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/bettercode`
- **Linux**: loesche `~/.cache/bettercode` (oder fuehre aus: `rm -rf ~/.cache/bettercode`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\bettercode`

3. Starte BetterCode Desktop neu.

---

### Server-Verbindungsprobleme beheben

BetterCode Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `bettercode.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `BETTERCODE_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `BETTERCODE_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt BetterCode Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit BetterCode-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

BetterCode Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer BetterCode in den OS-Einstellungen aktiviert sind, und
...
1. Beende BetterCode Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von BetterCode Desktop):

- `bettercode.settings.dat` (desktop default server URL)
- `bettercode.global.dat` und `bettercode.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit BetterCode hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `bettercode models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\bettercode`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. bettercode installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\bettercode`

2. Starte bettercode neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt bettercode, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
bettercode erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
