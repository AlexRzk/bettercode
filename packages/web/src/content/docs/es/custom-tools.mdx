...
description: Cree herramientas que LLM pueda llamar en bettercode.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de bettercode como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.bettercode/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/bettercode/tools/`.
...
```ts title=".bettercode/tools/database.ts" {1}
...
```ts title=".bettercode/tools/math.ts"
import { tool } from "@bettercode/plugin"
...
```ts title=".bettercode/tools/bash.ts"
import { tool } from "@bettercode/plugin"
...
```ts title=".bettercode/tools/project.ts" {8}
import { tool } from "@bettercode/plugin"
...
```python title=".bettercode/tools/add.py"
import sys
...
```ts title=".bettercode/tools/python-add.ts" {10}
import { tool } from "@bettercode/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".bettercode/tools/add.py")
