...
Puede agregar herramientas externas a BetterCode utilizando el _Model Context Protocol_ o MCP. BetterCode admite servidores locales y remotos.
...
Puede definir servidores MCP en su [BetterCode Config](https://bettercode.ai/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/bettercode`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
BetterCode maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, BetterCode:
...
Si el servidor requiere autenticación, BetterCode le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `bettercode mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, BetterCode almacenará los tokens de forma segura en `~/.local/share/bettercode/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en BetterCode, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración BetterCode como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar BetterCode a su cuenta Sentry.
