---
title: Complementos
description: Escriba sus propios complementos para extender BetterCode.
---

Los complementos le permiten extender BetterCode al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de BetterCode.
...
- `.bettercode/plugins/` - Complementos a nivel de proyecto
- `~/.config/bettercode/plugins/` - Complementos globales
...
```json title="bettercode.json"
{
  ...
  "plugin": ["bettercode-helicone-session", "bettercode-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/bettercode/node_modules/`.
...
1. Configuración global (`~/.config/bettercode/bettercode.json`)
2. Configuración del proyecto (`bettercode.json`)
3. Directorio global de complementos (`~/.config/bettercode/plugins/`)
4. Directorio de complementos del proyecto (`.bettercode/plugins/`)
...
```json title=".bettercode/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
BetterCode ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".bettercode/plugins/my-plugin.ts"
...
```js title=".bettercode/plugins/example.js"
...
- `client`: Un cliente SDK bettercode para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar bettercode.
...
```js title=".bettercode/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "bettercode"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio BetterCode, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que bettercode lea archivos `.env`:

```javascript title=".bettercode/plugins/env-protection.js"
...
```javascript title=".bettercode/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a bettercode:

```ts title=".bettercode/plugins/custom-tools.ts"
import { type Plugin, tool } from "@bettercode/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que bettercode puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para bettercode junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://bettercode.ai/docs/sdk) para obtener más detalles.
...
```ts title=".bettercode/plugins/compaction.ts"
import type { Plugin } from "@bettercode/plugin"
...
```ts title=".bettercode/plugins/custom-compaction.ts"
import type { Plugin } from "@bettercode/plugin"
