---
title: Proveedores
description: Usando cualquier proveedor LLM en BetterCode.
---

import config from "../../../../config.mjs"
export const console = config.console

BetterCode usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración BetterCode.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu BetterCode
configuración.
...
BetterCode Zen es una lista de modelos proporcionados por el equipo BetterCode que han sido
probado y verificado para funcionar bien con BetterCode. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con BetterCode Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione bettercode y diríjase a [bettercode.ai/auth](https://bettercode.ai/auth).
...
Funciona como cualquier otro proveedor en BetterCode y su uso es completamente opcional.

---

## BetterCode Go

BetterCode Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de BetterCode que han sido
probado y verificado para funcionar bien con BetterCode.

1. Ejecute el comando `/connect` en TUI, seleccione `BetterCode Go` y diríjase a [bettercode.ai/auth](https://bettercode.ai/zen).
...
Para usar Amazon Bedrock con BetterCode:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Establezca una de estas variables de entorno mientras ejecuta bettercode:
   ...
   Para una configuración persistente o específica del proyecto, utilice `bettercode.json`:
...
:::info
El uso de su suscripción Claude Pro/Max en BetterCode no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
Puedes configurar bettercode para usar modelos locales mediante [Atomic Chat](https://atomic.chat), una aplicación de escritorio que ejecuta LLMs locales detrás de un servidor API compatible con OpenAI (endpoint por defecto `http://127.0.0.1:1337/v1`).
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que bettercode funcione correctamente.
   :::
...
5. Ejecute el comando `/models` para seleccionar un modelo.
   ...
   También puede agregar modelos a través de su configuración bettercode.
...
2. Elija su método de autenticación:
   ...
   3. Nombre: `BetterCode`, Alcances: `api`
...
para almacenar el token en el almacenamiento de autenticación bettercode.
:::

##### Autohospedado GitLab

:::note[nota de cumplimiento]
BetterCode utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear BetterCode
Para usar solo su propia instancia alojada en GitLab, agregue lo siguiente a su
Archivo `bettercode.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Personalizar a través de `bettercode.json`:
...
Para utilizar su suscripción GitHub Copilot con bettercode:
...
Para utilizar Google Vertex AI con BetterCode:
...
2. Establezca las variables de entorno requeridas:
   ...
   Configúrelos mientras ejecuta bettercode.
...
En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de bettercode, siempre podrá configurarlo usted mismo.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación BetterCode enumerada como una sesión separada.
...
Puede configurar bettercode para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puede configurar bettercode para usar modelos locales a través de LM Studio.
...
Puedes configurar bettercode para usar modelos locales a través de Ollama.

:::tip
Ollama puede configurarse automáticamente para BetterCode. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/bettercode) para obtener más detalles.
:::
...
Para usar Ollama Cloud con BetterCode:
...
3. Copie la clave API para usarla en BetterCode.
...
6. **Importante**: Antes de usar modelos en la nube en BetterCode, debe obtener la información del modelo localmente:
...
BetterCode Zen es una lista de modelos probados y verificados proporcionada por el equipo BetterCode. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>BetterCode Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **BetterCode Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave BetterCode API.
...
4. Muchos modelos de OpenRouter están precargados de forma predeterminada, ejecute el comando `/models` para seleccionar el que desee.
   ...
   También puede agregar modelos adicionales a través de su configuración bettercode.
...
5. También puedes personalizarlos a través de tu configuración bettercode. A continuación se muestra un ejemplo de cómo especificar un proveedor.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con BetterCode:
...
También puede personalizar modelos a través de su configuración bettercode. A continuación se muestra un ejemplo de cómo especificar el orden de enrutamiento del proveedor.
...
:::tip
Puede utilizar cualquier proveedor compatible con OpenAI con bettercode. La mayoría de los proveedores de IA modernos ofrecen API compatibles con OpenAI.
:::
...
4. Cree o actualice su archivo `bettercode.json` en el directorio de su proyecto:
...
Los campos `limit` le permiten a BetterCode comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
1. **Verifique la configuración de autenticación**: Ejecute `bettercode auth list` para ver si las credenciales
...
2. Para proveedores personalizados, verifique la configuración bettercode y:
   - Asegúrese de que el ID del proveedor utilizado en el comando `/connect` coincida con el ID en su configuración bettercode.
