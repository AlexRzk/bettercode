---
title: Servidor
description: Interactuar con el servidor bettercode a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `bettercode serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente bettercode puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `bettercode.local` |
...
Configure `BETTERCODE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `bettercode`, o configure `BETTERCODE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `bettercode serve` como a `bettercode web`.
...
Cuando ejecuta `bettercode`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor bettercode para interactuar con bettercode mediante programación.
:::

Esta arquitectura permite que bettercode admita múltiples clientes y le permite interactuar con bettercode mediante programación.

Puede ejecutar `bettercode serve` para iniciar un servidor independiente. Si tienes el
bettercode TUI ejecutándose, `bettercode serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos BetterCode [IDE](/docs/ide).
...
El servidor bettercode expone las siguientes API.
