...
Las habilidades del agente permiten a BetterCode descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
BetterCode busca estas ubicaciones:

- Configuración del proyecto: `.bettercode/skills/<name>/SKILL.md`
- Configuración global: `~/.config/bettercode/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, BetterCode sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.bettercode/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/bettercode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.bettercode/skills/git-release/SKILL.md` así:
...
BetterCode enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `bettercode.json`:
...
**Para agentes integrados** (en `bettercode.json`):
