...
Con BetterCode puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, BetterCode usa nuestro propio tema `bettercode`.
...
BetterCode viene con varios temas integrados.
...
- Quiere que BetterCode coincida con la apariencia de su terminal
...
BetterCode admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/bettercode/themes/*.json` o `$XDG_CONFIG_HOME/bettercode/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.bettercode/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.bettercode/themes/*.json`
