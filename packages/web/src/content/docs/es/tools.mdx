...
Las herramientas permiten que LLM realice acciones en su código base. BetterCode viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en BetterCode.
...
:::note
Esta herramienta solo está disponible cuando `BETTERCODE_EXPERIMENTAL_LSP_TOOL=true` (o `BETTERCODE_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor BetterCode o cuando la variable de entorno `BETTERCODE_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar BetterCode:

```bash
BETTERCODE_ENABLE_EXA=1 bettercode
```
