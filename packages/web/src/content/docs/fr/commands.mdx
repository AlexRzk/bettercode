...
Créez `.bettercode/commands/test.md` :
...
Vous pouvez ajouter des commandes personnalisées via la configuration BetterCode ou en créant des fichiers markdown dans le répertoire `commands/`.

---

### JSON

Utilisez l'option `command` dans votre BetterCode [config](/docs/config) :
...
- Par projet : `.bettercode/commands/`
...
bettercode comprend plusieurs commandes intégrées comme `/init`, `/undo`, `/redo`, `/share`, `/help` ; [en savoir plus](/docs/tui#commands).
