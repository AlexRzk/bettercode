...
Vous pouvez ajouter des outils externes à BetterCode à l'aide du _Model Context Protocol_ ou MCP. BetterCode prend en charge les serveurs locaux et distants.
...
Vous pouvez définir des serveurs MCP dans votre [BetterCode Config](https://bettercode.ai/docs/config/) sous `mcp`. Ajoutez chaque MCP avec un nom unique. Vous pouvez faire référence à ce MCP par son nom lorsque vous demandez le LLM.
...
Les organisations peuvent fournir des serveurs MCP par défaut via leur point de terminaison `.well-known/bettercode`. Ces serveurs peuvent être désactivés par défaut, permettant aux utilisateurs de choisir ceux dont ils ont besoin.
...
BetterCode gère automatiquement l'authentification OAuth pour les serveurs MCP distants. Lorsqu'un serveur nécessite une authentification, BetterCode :
...
Si le serveur nécessite une authentification, BetterCode vous demandera de vous authentifier lorsque vous essayez de l'utiliser pour la première fois. Sinon, vous pouvez [déclencher manuellement le flux](#authenticating) avec `bettercode mcp auth <server-name>`.
...
La commande `mcp auth` ouvrira votre navigateur pour autorisation. Après votre autorisation, BetterCode stockera les jetons en toute sécurité dans `~/.local/share/bettercode/mcp-auth.json`.
...
Vos MCP sont disponibles sous forme d'outils dans BetterCode, aux côtés des outils intégrés. Vous pouvez donc les gérer via la configuration BetterCode comme n'importe quel autre outil.
...
Cela ouvrira une fenêtre de navigateur pour terminer le flux OAuth et connecter BetterCode à votre compte Sentry.
