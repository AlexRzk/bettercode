...
Les fournisseurs les plus populaires sont préchargés par défaut. Si vous avez ajouté les informations d'identification d'un fournisseur via la commande `/connect`, elles seront disponibles lorsque vous démarrerez BetterCode.
...
Voici plusieurs modèles qui fonctionnent bien avec BetterCode, sans ordre particulier. (Cette liste n’est pas exhaustive et n’est pas nécessairement à jour) :
...
Pour définir l'un d'entre eux comme modèle par défaut, vous pouvez définir la clé `model` dans votre configuration BetterCode.

```json title="bettercode.json" {3}
{
...
Ici, l'identifiant complet est `provider_id/model_id`. Par exemple, si vous utilisez [BetterCode Zen](/docs/zen), vous utiliserez `bettercode/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```jsonc title="bettercode.jsonc" {7-12,19-24}
{
  "$schema": "https://bettercode.ai/config.json",
  "provider": {
    "openai": {
...
```jsonc title="bettercode.jsonc" {6-21}
{
  "$schema": "https://bettercode.ai/config.json",
  "provider": {
    "bettercode": {
...
De nombreux modèles prennent en charge plusieurs variantes avec différentes configurations. BetterCode est livré avec des variantes par défaut intégrées pour les fournisseurs populaires.

### Variantes intégrées

BetterCode est livré avec des variantes par défaut pour de nombreux fournisseurs :
...
```jsonc title="bettercode.jsonc" {7-18}
{
  "$schema": "https://bettercode.ai/config.json",
  "provider": {
    "openai": {
...
Lorsque BetterCode démarre, il recherche les modèles dans l'ordre de priorité suivant :
...
2. La liste des modèles dans la configuration BetterCode.
