---
title: Plugins
description: Écrivez vos propres plugins pour étendre BetterCode.
---

Les plugins vous permettent d'étendre BetterCode en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de BetterCode.
...
- `.bettercode/plugins/` - Plugins au niveau du projet
- `~/.config/bettercode/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/bettercode/node_modules/`.
...
1. Configuration globale (`~/.config/bettercode/bettercode.json`)
2. Configuration du projet (`bettercode.json`)
3. Répertoire global des plugins (`~/.config/bettercode/plugins/`)
4. Répertoire des plugins du projet (`.bettercode/plugins/`)
...
BetterCode exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK bettercode pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre bettercode.
...
:::note
Si vous utilisez l'application de bureau BetterCode, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez bettercode de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à bettercode :
...
L'assistant `tool` crée un outil personnalisé que bettercode peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour bettercode aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://bettercode.ai/docs/sdk) pour plus de détails.
