---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en BetterCode.
---

import config from "../../../../config.mjs"
export const console = config.console

BetterCode utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration BetterCode.
...
en `~/.local/share/bettercode/auth.json`.

---

### Configuration

Vous pouvez personnaliser les fournisseurs via la section `provider` de votre BetterCode
configuration.
...
BetterCode Zen est une liste de modèles fournis par l'équipe BetterCode qui ont été
testé et vérifié pour fonctionner correctement avec BetterCode. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par BetterCode Zen.
:::

1. Exécutez la commande `/connect` dans le TUI, sélectionnez bettercode et dirigez-vous vers [bettercode.ai/auth](https://bettercode.ai/auth).
...
Il fonctionne comme n’importe quel autre fournisseur dans BetterCode et son utilisation est totalement facultative.

---

## BetterCode Go

BetterCode Go est un plan d'abonnement à faible coût qui offre un accès fiable aux modèles de codage ouverts populaires fournis par l'équipe BetterCode qui ont été
testé et vérifié pour fonctionner correctement avec BetterCode.

1. Exécutez la commande `/connect` dans le TUI, sélectionnez `BetterCode Go` et rendez-vous sur [bettercode.ai/auth](https://bettercode.ai/zen).
...
Pour utiliser Amazon Bedrock avec BetterCode :
...
Définissez l'une de ces variables d'environnement lors de l'exécution de bettercode :
...
Pour une configuration spécifique au projet ou persistante, utilisez `bettercode.json` :
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans BetterCode n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
Vous pouvez configurer bettercode pour utiliser des modèles locaux via [Atomic Chat](https://atomic.chat), une application de bureau qui exécute des LLM locaux derrière un serveur API compatible OpenAI (point de terminaison par défaut `http://127.0.0.1:1337/v1`).
...
2. Accédez à [Azure AI Foundry](https://ai.azure.com/) et déployez un modèle.
   ...
   :::note
   Le nom du déploiement doit correspondre au nom du modèle pour que bettercode fonctionne correctement.
   :::
...
Vous pouvez également ajouter des modèles via votre configuration bettercode.
...
pour stocker le jeton dans le stockage d'authentification bettercode.
:::

##### GitLab auto-hébergé

:::note[note de conformité]
BetterCode utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller BetterCode
pour utiliser uniquement votre propre instance hébergée par GitLab, ajoutez ce qui suit à votre
Fichier `bettercode.json`. Il est également recommandé de désactiver le partage de session.
...
Personnalisez via `bettercode.json` :
...
Pour utiliser votre abonnement GitHub Copilot avec bettercode :
...
Pour utiliser Google Vertex AI avec BetterCode :
...
Définissez-les lors de l'exécution de bettercode.
...
Si vous voyez une fonctionnalité ou un modèle d'Helicone qui n'est pas configuré automatiquement via bettercode, vous pouvez toujours le configurer vous-même.
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [bettercode-helicone-session](https://github.com/H2Shami/bettercode-helicone-session) pour enregistrer automatiquement chaque conversation BetterCode en tant que session dans Helicone.
...
Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation BetterCode répertoriée comme une session distincte.
...
Vous pouvez configurer bettercode pour utiliser des modèles locaux via l'utilitaire llama-server de [llama.cpp's](https://github.com/ggml-org/llama.cpp)
...
Vous pouvez configurer bettercode pour utiliser des modèles locaux via LM Studio.
...
Vous pouvez configurer bettercode pour utiliser des modèles locaux via Ollama.

:::tip
Ollama peut se configurer automatiquement pour BetterCode. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/bettercode) pour plus de détails.
:::
...
Pour utiliser Ollama Cloud avec BetterCode :
...
3. Copiez la clé API à utiliser dans BetterCode.
...
6. **Important** : Avant d'utiliser des modèles cloud dans BetterCode, vous devez extraire les informations du modèle localement :
...
BetterCode Zen est une liste de modèles testés et vérifiés fournie par l'équipe BetterCode. [En savoir plus](/docs/zen).

1. Connectez-vous à **<a href={console}>BetterCode Zen</a>** et cliquez sur **Créer une clé API**.

2. Exécutez la commande `/connect` et recherchez **BetterCode Zen**.

   ```txt
   /connect
   ```

3. Entrez votre clé BetterCode API.
...
Vous pouvez également ajouter des modèles supplémentaires via votre configuration bettercode.
...
5. Vous pouvez également les personnaliser via votre configuration bettercode. Voici un exemple de spécification d'un fournisseur
...
Vous pouvez également personnaliser les modèles via votre configuration bettercode. Voici un exemple de spécification de l'ordre de routage du fournisseur.
...
:::tip
Vous pouvez utiliser n'importe quel fournisseur compatible OpenAI avec bettercode. La plupart des fournisseurs d'IA modernes proposent des API compatibles OpenAI.
:::
...
4. Créez ou mettez à jour votre fichier `bettercode.json` dans le répertoire de votre projet :
...
Les champs `limit` permettent à BetterCode de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
...
1. **Vérifiez la configuration de l'authentification** : exécutez `bettercode auth list` pour voir si les informations d'identification
...
2. Pour les fournisseurs personnalisés, vérifiez la configuration bettercode et :
   - Assurez-vous que l'ID du fournisseur utilisé dans la commande `/connect` correspond à l'ID de votre configuration bettercode.
