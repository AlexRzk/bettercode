---
title: Serveur
description: Interagissez avec le serveur bettercode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `bettercode serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client bettercode peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `bettercode.local` |
...
Définissez `BETTERCODE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `bettercode`, ou définissez `BETTERCODE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `bettercode serve` et à `bettercode web`.
...
Lorsque vous exécutez `bettercode`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur bettercode pour interagir avec bettercode par programmation.
:::

Cette architecture permet à bettercode de prendre en charge plusieurs clients et vous permet d'interagir avec bettercode par programme.

Vous pouvez exécuter `bettercode serve` pour démarrer un serveur autonome. Si vous avez le
bettercode TUI en cours d'exécution, `bettercode serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins BetterCode [IDE](/docs/ide).
...
Le serveur bettercode expose les API suivantes.
