...
Avec BetterCode, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, BetterCode utilise notre propre thème `bettercode`.
...
BetterCode est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que BetterCode corresponde à l'apparence de leur terminal
...
BetterCode prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/bettercode/themes/*.json` ou `$XDG_CONFIG_HOME/bettercode/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.bettercode/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.bettercode/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/bettercode/themes
vim ~/.config/bettercode/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .bettercode/themes
vim .bettercode/themes/my-theme.json
```
...
{
  "$schema": "https://bettercode.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
