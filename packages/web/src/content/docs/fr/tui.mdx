---
title: TUI
description: Utilisation de l’interface utilisateur du terminal BetterCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

BetterCode fournit une interface de terminal interactif ou TUI pour travailler sur vos projets avec un LLM.

L’exécution de BetterCode démarre le TUI pour le répertoire actuel.
...
Lorsque vous utilisez le BetterCode TUI, vous pouvez taper `/` suivi d'un nom de commande pour exécuter rapidement des actions. Par exemple:
...
Ajoutez un fournisseur à BetterCode. Vous permet de sélectionner parmi les fournisseurs disponibles et d’ajouter leurs clés API.
...
Quittez BetterCode. *Alias* : `/quit`, `/q`
...
Ceci est séparé de `bettercode.json`, qui configure le comportement du serveur/d'exécution.
...
- `leader_timeout` - Contrôle la durée pendant laquelle BetterCode attend après la leader key. Valeur par défaut : `2000`.
...
- `sound_pack` - ID du sound pack à utiliser. Valeur par défaut : `bettercode.default`.
