...
BetterCode Go è un abbonamento a basso costo — **5 $ per il primo mese**, poi **10 $/mese** — che ti offre un accesso affidabile ai popolari modelli di programmazione aperti.

Go funziona come qualsiasi altro provider in BetterCode. Ti abboni a BetterCode Go e
ottieni la tua chiave API. È **completamente facoltativo** e non hai bisogno di usarlo per
utilizzare BetterCode.
...
BetterCode Go ti dà accesso a questi modelli a **5 $ per il primo mese**, poi a **10 $/mese**.

---

## Come funziona

BetterCode Go funziona come qualsiasi altro provider in BetterCode.

1. Accedi a **<a href={console}>BetterCode Zen</a>**, ti abboni a Go e
   copi la tua chiave API.
2. Esegui il comando `/connect` nella TUI, selezioni `BetterCode Go` e incolli
   la tua chiave API.
3. Esegui `/models` nella TUI per vedere l'elenco dei modelli disponibili tramite Go.

:::note
Solo un membro per workspace può abbonarsi a BetterCode Go.
:::
...
BetterCode Go include i seguenti limiti:
...
Il [model id](/docs/config/#models) nella tua BetterCode config
utilizza il formato `bettercode-go/<model-id>`. Ad esempio, per Kimi K2.7 Code, useresti
`bettercode-go/kimi-k2.7-code` nella tua configurazione.
...
Abbiamo creato BetterCode Go per:
...
4. Non avere **nessun lock-in**, permettendoti di utilizzare anche qualsiasi altro provider con BetterCode.
