...
Puoi aggiungere strumenti esterni a BetterCode usando il _Model Context Protocol_, o MCP. BetterCode supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [BetterCode Config](https://bettercode.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/bettercode`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
BetterCode gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, BetterCode:
...
Se il server richiede autenticazione, BetterCode ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `bettercode mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, BetterCode memorizzera' i token in modo sicuro in `~/.local/share/bettercode/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in BetterCode insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione BetterCode come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare BetterCode al tuo account Sentry.
