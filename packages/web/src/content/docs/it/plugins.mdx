---
title: Plugin
description: Scrivi plugin per estendere BetterCode.
---

I plugin ti permettono di estendere BetterCode agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di BetterCode.
...
- `.bettercode/plugins/` - plugin a livello progetto
- `~/.config/bettercode/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/bettercode/node_modules/`.
...
1. Config globale (`~/.config/bettercode/bettercode.json`)
2. Config di progetto (`bettercode.json`)
3. Directory plugin globale (`~/.config/bettercode/plugins/`)
4. Directory plugin di progetto (`.bettercode/plugins/`)
...
BetterCode esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di bettercode per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere bettercode.
...
:::note
Se usi l'app desktop di BetterCode, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a bettercode di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a bettercode:
...
L'helper `tool` crea uno strumento personalizzato che bettercode può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in bettercode insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://bettercode.ai/docs/sdk) per i dettagli.
