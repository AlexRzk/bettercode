---
title: Server
description: Interagisci con il server bettercode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `bettercode serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client bettercode.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `bettercode.local` |
...
Imposta `BETTERCODE_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `bettercode`, oppure imposta `BETTERCODE_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `bettercode serve` sia per `bettercode web`.
...
Quando esegui `bettercode` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server bettercode per interagire con bettercode in modo programmatico.
:::

Questa architettura permette a bettercode di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `bettercode serve` per avviare un server standalone. Se la TUI di bettercode e gia in esecuzione, `bettercode serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di BetterCode.
...
Il server bettercode espone le seguenti API.
