...
Le skill degli agenti permettono a BetterCode di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
BetterCode cerca in queste posizioni:

- Config di progetto: `.bettercode/skills/<name>/SKILL.md`
- Config globale: `~/.config/bettercode/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, BetterCode risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.bettercode/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/bettercode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.bettercode/skills/git-release/SKILL.md` cosi':
...
BetterCode elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `bettercode.json`:
...
**Per agenti integrati** (in `bettercode.json`):
