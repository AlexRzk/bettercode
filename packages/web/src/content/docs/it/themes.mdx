...
Con BetterCode puoi scegliere tra diversi temi integrati, usare un tema che si adatta al tema del tuo terminale oppure definire un tema personalizzato.

Per impostazione predefinita, BetterCode usa il tema `bettercode`.
...
BetterCode include diversi temi integrati.
...
- Vuole che BetterCode corrisponda all'aspetto del terminale
...
BetterCode supporta un sistema di temi flessibile basato su JSON che permette di creare e personalizzare temi facilmente.
...
2. **Directory di configurazione utente** - in `~/.config/bettercode/themes/*.json` o `$XDG_CONFIG_HOME/bettercode/themes/*.json`
3. **Directory root del progetto** - in `<project-root>/.bettercode/themes/*.json`
4. **Directory di lavoro corrente** - in `./.bettercode/themes/*.json`
