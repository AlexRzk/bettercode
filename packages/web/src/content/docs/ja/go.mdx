...
BetterCode Goは低価格のサブスクリプションで、**初月は5ドル**、その後は**月額10ドル**で、人気のオープンなコーディングモデルに安定してアクセスできます。

GoはBetterCodeの他のプロバイダーと同様に機能します。BetterCode GoをサブスクライブしてAPIキーを取得します。これは**完全に任意**であり、BetterCodeを使用するために必須ではありません。
...
:::tip
BetterCodeでうまく動作する一部のモデルとプロバイダーをテストしました。
:::
...
BetterCode Goを使用すると、これらのモデルに**初月は5ドル**、その後は**月額10ドル**でアクセスできます。

---

## 仕組み

BetterCode Goは、BetterCodeの他のプロバイダーと同様に機能します。

1. **<a href={console}>BetterCode Zen</a>**にサインインし、GoをサブスクライブしてAPIキーをコピーします。
2. TUIで`/connect`コマンドを実行し、`BetterCode Go`を選択して、APIキーを貼り付けます。
3. TUIで`/models`を実行すると、Goを通じて利用可能なモデルのリストが表示されます。

:::note
BetterCode Goをサブスクライブできるのは、1つのワークスペースにつき1メンバーのみです。
:::
...
BetterCode設定の[model id](/docs/config/#models)は、`bettercode-go/<model-id>`という形式を使用します。たとえば、Kimi K2.7 Codeの場合は、設定で`bettercode-go/kimi-k2.7-code`を使用します。
...
4. BetterCodeで他のプロバイダーも使用できるようにすることで、**ベンダーロックインをなくす**。
