---
title: サーバー
description: HTTP 経由で BetterCode サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`bettercode serve` コマンドは、BetterCode クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `bettercode.local` |
...
HTTP 基本認証でサーバーを保護するには、`BETTERCODE_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `bettercode` になるか、`BETTERCODE_SERVER_USERNAME` を設定してオーバーライドします。これは、`bettercode serve` と `bettercode web` の両方に当てはまります。
...
`bettercode` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
BetterCode サーバーを使用して、プログラムで BetterCode と対話します。
:::

`bettercode serve` を実行してスタンドアロンサーバーを起動できます。すでに BetterCode TUI を実行している場合でも、`bettercode serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、BetterCode [IDE](/docs/ide) プラグインによって使用されます。
