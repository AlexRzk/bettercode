---
title: LSP Servers
description: BetterCode integrates with your LSP servers.
---

BetterCode can integrate with Language Server Protocol (LSP) servers to use diagnostics as feedback for the agent.

---

## Built-in

BetterCode comes with several built-in LSP servers for popular languages:
...
:::note
You can disable automatic LSP server downloads by setting the `BETTERCODE_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When LSP is enabled and bettercode opens a file, it:
...
You can enable and customize LSP servers through the `lsp` section in your bettercode config.
...
```json title="bettercode.json" {5-8}
{
  "$schema": "https://bettercode.ai/config.json",
  "lsp": {
    "rust": {
...
```json title="bettercode.json" {5-13}
{
  "$schema": "https://bettercode.ai/config.json",
  "lsp": {
    "custom-lsp": {
