...
You can add external tools to BetterCode using the _Model Context Protocol_, or MCP. BetterCode supports both local and remote servers.
...
You can define MCP servers in your [BetterCode Config](https://bettercode.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/bettercode` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
BetterCode automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, BetterCode will:
...
If the server requires authentication, BetterCode will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `bettercode mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, BetterCode will store the tokens securely in `~/.local/share/bettercode/mcp-auth.json`.
...
Your MCPs are available as tools in BetterCode, alongside built-in tools. So you can manage them through the BetterCode config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect BetterCode to your Sentry account.
