...
Det er to typer agenter i BetterCode; primære agenter og underagenter.
...
BetterCode kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
BetterCode kommer med tre innebygde underagenter, **General**, **Explore** og **Scout**. Vi skal se på dette nedenfor.

---

## Innebygd

BetterCode kommer med to innebygde primære agenter og tre innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `bettercode.json`:
...
- Per prosjekt: `.bettercode/agents/`
...
Hvis ingen temperatur er spesifisert, bruker BetterCode modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale BetterCode-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i BetterCode-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [BetterCode Zen](/docs/zen), vil du bruke `bettercode/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `bettercode models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/anomalyco/bettercode).
:::
