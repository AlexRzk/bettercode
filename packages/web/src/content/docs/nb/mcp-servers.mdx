...
Du kan legge til eksterne verktøy i BetterCode ved å bruke _Model Context Protocol_ eller MCP. BetterCode støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [BetterCode Config](https://bettercode.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="bettercode.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/bettercode`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
```jsonc title="bettercode.jsonc" {15}
{
  "$schema": "https://bettercode.ai/config.json",
  "mcp": {
    "my-local-mcp-server": {
...
BetterCode håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil BetterCode:
...
Hvis serveren krever autentisering, vil BetterCode be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `bettercode mcp auth <server-name>`.
...
```json title="bettercode.json" {7-11}
{
  "$schema": "https://bettercode.ai/config.json",
  "mcp": {
    "my-oauth-server": {
...
```bash
bettercode mcp auth my-oauth-server
```
...
```bash
bettercode mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil BetterCode lagre tokens sikkert i `~/.local/share/bettercode/mcp-auth.json`.
...
bettercode mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i BetterCode, sammen med innebygde verktøy. Så du kan administrere dem gjennom BetterCode-konfigurasjonen som alle andre verktøy.
...
```json title="bettercode.json" {11, 14-18}
{
  "$schema": "https://bettercode.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```json title="bettercode.json" {4-8}
{
  "$schema": "https://bettercode.ai/config.json",
  "mcp": {
    "sentry": {
...
```bash
bettercode mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble BetterCode til Sentry-kontoen din.
...
```json title="bettercode.json" {4-7}
{
  "$schema": "https://bettercode.ai/config.json",
  "mcp": {
    "context7": {
...
```json title="bettercode.json" {7-9}
{
  "$schema": "https://bettercode.ai/config.json",
  "mcp": {
    "context7": {
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver BetterCode-samtale oppført som en separat økt.
