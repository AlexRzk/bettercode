...
description: Skriv dine egne programtillegg for å utvide BetterCode.
---

Plugins lar deg utvide BetterCode ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til BetterCode.
...
- `.bettercode/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/bettercode/node_modules/`.
...
1. Global konfigurasjon (`~/.config/bettercode/bettercode.json`)
2. Prosjektkonfigurasjon (`bettercode.json`)
3. Global plugin-katalog (`~/.config/bettercode/plugins/`)
4. Prosjektpluginkatalog (`.bettercode/plugins/`)
...
BetterCode kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En BetterCode SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide BetterCode.
...
:::note
Hvis du bruker BetterCode-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre BetterCode fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til BetterCode:
...
`tool`-hjelperen lager et tilpasset verktøy som BetterCode kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for BetterCode sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://bettercode.ai/docs/sdk) for detaljer.
