---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i BetterCode.
---

import config from "../../../../config.mjs"
export const console = config.console

BetterCode bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i BetterCode-konfigurasjonen.
...
i `~/.local/share/bettercode/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i BetterCode
konfig.
...
BetterCode Zen er en liste over modeller levert av BetterCode-teamet som har vært
testet og verifisert for å fungere godt med BetterCode. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med BetterCode Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `BetterCode Zen` og gå til [bettercode.ai/auth](https://bettercode.ai/zen).
...
Det fungerer som alle andre leverandører i BetterCode og er helt valgfritt å bruke.

---

## BetterCode Go

BetterCode Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av BetterCode-teamet som har vært
testet og verifisert for å fungere godt med BetterCode.

1. Kjør kommandoen `/connect` i TUI, velg `BetterCode Go`, og gå til [bettercode.ai/auth](https://bettercode.ai/zen).
...
Slik bruker du Amazon Bedrock med BetterCode:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører BetterCode:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `bettercode.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i bettercode støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere bettercode til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at BetterCode skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom BetterCode-konfigurasjonen.
...
for å lagre token i BetterCode auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
BetterCode bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse BetterCode
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`bettercode.json` fil. Det anbefales også å deaktivere øktdeling.
...
Tilpass gjennom `bettercode.json`:
...
Slik bruker du GitHub Copilot-abonnementet med BetterCode:
...
Slik bruker du Google Vertex AI med BetterCode:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører BetterCode.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom BetterCode, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [bettercode-helicone-session](https://github.com/H2Shami/bettercode-helicone-session) plugin for automatisk å logge hver BetterCode-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver BetterCode-samtale oppført som en separat økt.
...
Du kan konfigurere BetterCode for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere BetterCode for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere BetterCode for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for BetterCode. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/bettercode) for detaljer.
:::
...
Slik bruker du Ollama Cloud med BetterCode:
...
3. Kopier API-nøkkelen for bruk i BetterCode.
...
6. **Viktig**: Før du bruker skymodeller i BetterCode, må du hente modellinformasjonen lokalt:
...
BetterCode Zen er en liste over testede og verifiserte modeller levert av BetterCode-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>BetterCode Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **BetterCode Zen**.

   ```txt
   /connect
   ```

3. Skriv inn BetterCode API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom BetterCode-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom BetterCode-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med BetterCode:
...
Du kan også tilpasse modeller gjennom BetterCode-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med BetterCode. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `bettercode.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar BetterCode forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `bettercode auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk BetterCode-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i BetterCode-konfigurasjonen.
