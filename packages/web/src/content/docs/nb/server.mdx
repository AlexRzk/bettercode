---
title: Tjener
description: Samhandle med BetterCode-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `bettercode serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en BetterCode-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `bettercode.local` |
...
Sett `BETTERCODE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `bettercode` som standard, eller sett `BETTERCODE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `bettercode serve` og `bettercode web`.
...
Når du kjører `bettercode` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk BetterCode-serveren til å samhandle med BetterCode programmatisk.
:::

Denne arkitekturen lar BetterCode støtte flere klienter og lar deg samhandle med BetterCode programmatisk.

Du kan kjøre `bettercode serve` for å starte en frittstående server. Hvis du har
BetterCode TUI kjører, vil `bettercode serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av BetterCode [IDE](/docs/ide) plugins.
...
bettercode-serveren viser følgende APIer.
