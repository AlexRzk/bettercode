...
Agentferdigheter lar BetterCode oppdage gjenbrukbare instruksjoner fra repo- eller hjemmekatalogen din.
...
bettercode søker etter disse stedene:

- Prosjektkonfigurasjon: `.bettercode/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/bettercode/skills/<name>/SKILL.md`
...
For prosjektlokale stier går bettercode opp fra din nåværende arbeidskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.bettercode/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/bettercode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.bettercode/skills/git-release/SKILL.md` slik:
...
bettercode viser tilgjengelige ferdigheter i `skill`-verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved å bruke mønsterbaserte tillatelser i `bettercode.json`:
...
**For innebygde agenter** (i `bettercode.json`):
