...
BetterCode Go to niskokosztowa subskrypcja — **5 $ za pierwszy miesiąc**, a następnie **10 $/miesiąc** — która zapewnia niezawodny dostęp do popularnych otwartych modeli do kodowania.

Go działa jak każdy inny dostawca w BetterCode. Subskrybujesz BetterCode Go i
...
BetterCode Go daje Ci dostęp do tych modeli za **5 $ za pierwszy miesiąc**, a następnie **10 $/miesiąc**.

---

## Jak to działa

BetterCode Go działa jak każdy inny dostawca w BetterCode.

1. Logujesz się do **<a href={console}>BetterCode Zen</a>**, subskrybujesz Go i
   kopiujesz swój klucz API.
2. Uruchamiasz komendę `/connect` w TUI, wybierasz `BetterCode Go` i wklejasz
   swój klucz API.
3. Uruchom `/models` w TUI, aby zobaczyć listę modeli dostępnych w ramach Go.

:::note
Tylko jeden członek na obszar roboczy (workspace) może zasubskrybować BetterCode Go.
:::
...
BetterCode Go zawiera następujące limity:
...
[ID modelu](/docs/config/#models) w Twojej konfiguracji BetterCode
używa formatu `bettercode-go/<model-id>`. Na przykład dla Kimi K2.7 Code należy użyć
`bettercode-go/kimi-k2.7-code` w swojej konfiguracji.
...
Stworzyliśmy BetterCode Go, aby:
...
4. Zapewnić **brak ograniczeń (no lock-in)**, pozwalając na korzystanie z dowolnego innego dostawcy wraz z BetterCode.
