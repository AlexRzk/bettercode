...
Możesz dodać dodatkowe informacje do bettercode za pomocą _Model Context Protocol_, w skrócie MCP. bettercode obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [bettercode Config](https://bettercode.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/bettercode`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
bettercode automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, bettercode:
...
Jeśli serwer wymaga uwierzytelnienia, bettercode poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `bettercode mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji bettercode bezpieczne przechowa tokeny w `~/.local/share/bettercode/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w bettercode, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie bettercode, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć bettercode z kontem Sentry.
