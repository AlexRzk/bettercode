---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć bettercode.
---

Wtyczki umożliwiające wyłączenie bettercode poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania bettercode.
...
- `.bettercode/plugins/` - Wtyczki na poziomie projektu
- `~/.config/bettercode/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/bettercode/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/bettercode/bettercode.json`)
2. Konfiguracja projektu (`bettercode.json`)
3. Globalny katalog wtyczek (`~/.config/bettercode/plugins/`)
4. Katalog wtyczek projektu (`.bettercode/plugins/`)
...
bettercode uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z bettercode do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do bettercode.
...
:::note
Jeśli korzystasz z aplikacji komputerowej bettercode, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj bettercode czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do bettercode:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod bettercode. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://bettercode.ai/docs/sdk).
