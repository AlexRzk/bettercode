---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w bettercode.
---

import config from "../../../../config.mjs"
export const console = config.console

bettercode używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji bettercode.
...
w `~/.local/share/bettercode/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim bettercode
config.
...
BetterCode Zen to lista modeli dostarczonych przez zespół bettercode, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z bettercode. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od BetterCode Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `BetterCode Zen` i przejdź do [bettercode.ai/auth](https://bettercode.ai/zen).
...
Działa jak każdy inny dostawca w bettercode i jest całkowicie opcjonalny w użyciu.

---

## BetterCode Go

BetterCode Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół bettercode, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z bettercode.

1. Uruchom polecenie `/connect` w TUI, wybierz `BetterCode Go` i przejdź do [bettercode.ai/auth](https://bettercode.ai/zen).
...
Aby używać Amazon Bedrock z bettercode:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania bettercode:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `bettercode.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w bettercode nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować bettercode do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod bettercode działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji bettercode.
...
bettercode używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować bettercode
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`bettercode.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Aby korzystać z subskrypcji GitHub Copilot z kodem bettercode:
...
Aby używać Google Vertex AI z bettercode:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania bettercode.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą bettercode, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [bettercode-helicone-session](https://github.com/H2Shami/bettercode-helicone-session), aby automatycznie rejestrować każdą konwersację bettercode jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja bettercode będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować bettercode tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować bettercode tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować bettercode tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem bettercode. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/bettercode).
:::
...
Aby korzystać z Ollama Cloud z bettercode:
...
3. Skopiuj klucz API do użycia w bettercode.
...
6. **Ważne**: Przed użyciem modeli chmurowych w bettercode musisz pobrać informacje o modelu lokalnie:
...
BetterCode Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół bettercode. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>BetterCode Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **BetterCode Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API bettercode.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji bettercode.
...
5. Możesz je także dostosować za pomocą konfiguracji bettercode. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z bettercode:
...
Możesz także dostosować modele za pomocą konfiguracji bettercode. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z bettercode. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `bettercode.json` w katalogu projektu:
...
Pola `limit` pozwalają bettercode zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `bettercode auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację bettercode i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji bettercode.
