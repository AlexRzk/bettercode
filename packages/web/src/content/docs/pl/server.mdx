---
title: Serwer
description: Interakcja z serwerem bettercode za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `bettercode serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient bettercode.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `bettercode.local` |
...
Ustaw `BETTERCODE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `bettercode` lub ustaw `BETTERCODE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `bettercode serve`, jak i `bettercode web`.
...
Kiedy uruchomisz `bettercode`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera bettercode do programowej interakcji z bettercode.
:::

Ta architektura umożliwia obsługę wielu klientów przez bettercode i programową interakcję z kodem otwartym.

Możesz uruchomić `bettercode serve`, aby uruchomić samodzielny serwer. Jeśli masz
bettercode TUI działa, `bettercode serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki bettercode [IDE](/docs/ide).
...
Serwer bettercode udostępnia następujące interfejsy API.
