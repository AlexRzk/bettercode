...
Umiejętności agenta pozwalają bettercode odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
bettercode przeszukuje te lokalizacje:

- Project config: `.bettercode/skills/<name>/SKILL.md`
- Global config: `~/.config/bettercode/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu bettercode przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.bettercode/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/bettercode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.bettercode/skills/git-release/SKILL.md` w ten sposób:
...
bettercode wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `bettercode.json`:
...
**Dla agentów wbudowanych** (w `bettercode.json`):
