...
Dzięki bettercode możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie bettercode używa naszego własnego motywu `bettercode`.
...
bettercode ma kilka wbudowanych motywów.
...
- Chcą, aby BetterCode pasował do wyglądu ich terminala
...
bettercode obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/bettercode/themes/*.json` lub `$XDG_CONFIG_HOME/bettercode/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.bettercode/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.bettercode/themes/*.json`
