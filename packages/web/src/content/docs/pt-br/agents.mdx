...
Existem dois tipos de agentes no bettercode; agentes primários e subagentes.
...
bettercode vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
BetterCode vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

BetterCode vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do BetterCode, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Configure os agentes em seu arquivo de configuração `bettercode.json`:
...
- Global: `~/.config/bettercode/agents/`
- Por projeto: `.bettercode/agents/`

```markdown title="~/.config/bettercode/agents/review.md"
---
...
Se nenhuma temperatura for especificada, o bettercode usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do bettercode quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do bettercode usa o formato `provider/model-id`. Por exemplo, se você estiver usando [BetterCode Zen](/docs/zen), você usaria `bettercode/gpt-5.1-codex` para GPT 5.1 Codex.
...
```json title="bettercode.json" {3-6,9-12}
{
  "$schema": "https://bettercode.ai/config.json",
  "tools": {
    "write": true,
...
```json title="bettercode.json" {3-5,8-10}
{
  "$schema": "https://bettercode.ai/config.json",
  "permission": {
    "edit": "deny"
...
```json title="bettercode.json" {6,7}
{
  "agent": {
    "deep-thinker": {
...
:::tip
Execute `bettercode models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/anomalyco/bettercode).
:::
...
```markdown title="~/.config/bettercode/agents/docs-writer.md"
---
...
```markdown title="~/.config/bettercode/agents/security-auditor.md"
---
