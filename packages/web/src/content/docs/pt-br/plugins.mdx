---
title: Plugins
description: Escreva seus próprios plugins para estender o bettercode.
---

Plugins permitem que você estenda o bettercode conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do bettercode.
...
- `.bettercode/plugins/` - Plugins em nível de projeto
- `~/.config/bettercode/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/bettercode/node_modules/`.
...
1. Configuração global (`~/.config/bettercode/bettercode.json`)
2. Configuração do projeto (`bettercode.json`)
3. Diretório de plugins global (`~/.config/bettercode/plugins/`)
4. Diretório de plugins do projeto (`.bettercode/plugins/`)
...
O bettercode executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do bettercode para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o bettercode.
...
:::note
Se você estiver usando o aplicativo desktop bettercode, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o bettercode de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao bettercode:
...
O helper `tool` cria uma ferramenta personalizada que o bettercode pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o bettercode junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://bettercode.ai/docs/sdk) para detalhes.
