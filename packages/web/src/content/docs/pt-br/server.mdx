---
title: Servidor
description: Interaja com o servidor bettercode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `bettercode serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente bettercode pode usar.
...
```bash
bettercode serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `bettercode.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
bettercode serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `BETTERCODE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `bettercode`, ou defina `BETTERCODE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `bettercode serve` quanto ao `bettercode web`.

```bash
BETTERCODE_SERVER_PASSWORD=your-password bettercode serve
```
...
Quando você executa `bettercode`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor bettercode para interagir com o bettercode programaticamente.
:::

Essa arquitetura permite que o bettercode suporte múltiplos clientes e permite que você interaja com o bettercode programaticamente.

Você pode executar `bettercode serve` para iniciar um servidor autônomo. Se você tiver o TUI do bettercode em execução, `bettercode serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do bettercode [IDE](/docs/ide).
...
O servidor bettercode expõe as seguintes APIs.
