...
As habilidades do agente permitem que o bettercode descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O bettercode pesquisa nesses locais:

- Configuração do projeto: `.bettercode/skills/<name>/SKILL.md`
- Configuração global: `~/.config/bettercode/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o bettercode sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.bettercode/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/bettercode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.bettercode/skills/git-release/SKILL.md` assim:
...
O bettercode lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `bettercode.json`:
...
**Para agentes embutidos** (em `bettercode.json`):
