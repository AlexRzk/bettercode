...
Com o bettercode, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o bettercode usa nosso próprio tema `bettercode`.
...
O bettercode vem com vários temas embutidos.
...
- Querem que o bettercode corresponda à aparência do seu terminal
...
O bettercode suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/bettercode/themes/*.json` ou `$XDG_CONFIG_HOME/bettercode/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.bettercode/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.bettercode/themes/*.json`
