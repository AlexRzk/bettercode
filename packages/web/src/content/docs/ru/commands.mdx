...
Создайте `.bettercode/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию bettercode или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем bettercode [config](/docs/config):
...
- Глобальный: `~/.config/bettercode/commands/`
- Для каждого проекта: `.bettercode/commands/`
...
bettercode включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
