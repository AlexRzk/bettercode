---
title: Server
description: Interact with bettercode server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `bettercode serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an bettercode client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `bettercode.local` |
...
Set `BETTERCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `bettercode`, or set `BETTERCODE_SERVER_USERNAME` to override it. This applies to both `bettercode serve` and `bettercode web`.
...
When you run `bettercode` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the bettercode server to interact with bettercode programmatically.
:::

This architecture lets bettercode support multiple clients and allows you to interact with bettercode programmatically.

You can run `bettercode serve` to start a standalone server. If you have the
bettercode TUI running, `bettercode serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the BetterCode [IDE](/docs/ide) plugins.
...
The bettercode server exposes the following APIs.
