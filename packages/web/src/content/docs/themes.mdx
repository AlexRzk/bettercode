...
With BetterCode you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, BetterCode uses our own `bettercode` theme.
...
BetterCode comes with several built-in themes.
...
- Want BetterCode to match their terminal's appearance
...
BetterCode supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/bettercode/themes/*.json` or `$XDG_CONFIG_HOME/bettercode/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.bettercode/themes/*.json`
4. **Current working directory** - Defined in `./.bettercode/themes/*.json`
