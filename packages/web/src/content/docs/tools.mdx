...
Tools allow the LLM to perform actions in your codebase. BetterCode comes with a set of built-in tools, but you can extend it with [custom tools](/docs/custom-tools) or [MCP servers](/docs/mcp-servers).
...
Here are all the built-in tools available in BetterCode.
...
:::note
This tool is only available when `BETTERCODE_EXPERIMENTAL_LSP_TOOL=true` (or `BETTERCODE_EXPERIMENTAL=true`).
:::
...
:::note
This tool is only available when using the BetterCode provider or when the `BETTERCODE_ENABLE_EXA` environment variable is set to any truthy value (e.g., `true` or `1`).

To enable when launching BetterCode:
