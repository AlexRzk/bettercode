...
bettercode'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
bettercode, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
BetterCode, **General**, **Explore** ve **Scout** olmak üzere üç yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

BetterCode iki yerleşik birincil agent ve üç yerleşik alt agent ile birlikte gelir.
...
Agent'ları `bettercode.json` yapılandırma dosyanızda yapılandırın:
...
- Proje başına: `.bettercode/agents/`
...
Sıcaklık belirtilmezse bettercode modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global bettercode yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
bettercode yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [BetterCode Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `bettercode/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `bettercode models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/anomalyco/bettercode).
:::
