...
description: bettercode dile özgü biçimlendiriciler kullanır.
---

bettercode, dosyaları dile özgü formatlayıcılar kullanılarak yazıldıktan veya düzenlendikten sonra otomatik olarak formatlar. Bu, oluşturulan kodun projenizin kod stillerine uymasını sağlar.

---

## Yerleşik

bettercode, popüler diller ve çerçeveler için çeşitli yerleşik biçimlendiricilerle birlikte gelir. Aşağıda ihtiyaç duyduğu biçimlendiricilerin, desteklenen dosya uzantılarının ve komutların veya yapılandırma seçeneklerinin bir listesi bulunmaktadır.
...
Yani eğer projenizin `package.json` dosyasında `prettier` varsa, bettercode bunu otomatik olarak kullanacaktır.

---

## Nasıl çalışır?

bettercode bir dosyayı yazdığında veya düzenlediğinde:
...
Biçimlendiricileri bettercode yapılandırmanızdaki `formatter` bölümü aracılığıyla özelleştirebilirsiniz.
