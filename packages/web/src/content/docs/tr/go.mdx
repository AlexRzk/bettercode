...
BetterCode Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, BetterCode'daki diğer sağlayıcılar gibi çalışır. BetterCode Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve BetterCode'u kullanmak için bunu kullanmanıza gerek yoktur.
...
BetterCode Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

BetterCode Go, BetterCode'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>BetterCode Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `BetterCode Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye BetterCode Go'ya abone olabilir.
:::
...
BetterCode Go aşağıdaki limitleri içerir:
...
BetterCode yapılandırmanızdaki [model id](/docs/config/#models) formatı `bettercode-go/<model-id>` şeklindedir. Örneğin, Kimi K2.7 Code için yapılandırmanızda `bettercode-go/kimi-k2.7-code` kullanmalısınız.
...
BetterCode Go'yu şu amaçlarla oluşturduk:
...
4. BetterCode ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
