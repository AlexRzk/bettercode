...
_Model Bağlam Protokolü_ veya MCP kullanarak bettercode'a harici araçlar ekleyebilirsiniz. bettercode hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [bettercode Config](https://bettercode.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/bettercode` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
bettercode, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde bettercode şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa bettercode, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `bettercode mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından bettercode, belirteçleri `~/.local/share/bettercode/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra bettercode'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi bettercode yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve bettercode'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
