---
title: Eklentiler
description: BetterCode'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek BetterCode'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya BetterCode'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.bettercode/plugins/` - Proje düzeyinde eklentiler
- `~/.config/bettercode/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/bettercode/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/bettercode/bettercode.json`)
2. Project config (`bettercode.json`)
3. Global eklenti dizini (`~/.config/bettercode/plugins/`)
4. Proje eklenti dizini (`.bettercode/plugins/`)
...
BetterCode bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir BetterCode SDK istemcisi.
...
BetterCode'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
BetterCode masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
BetterCode'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca BetterCode'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, BetterCode'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra BetterCode'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://bettercode.ai/docs/sdk) bakın.
