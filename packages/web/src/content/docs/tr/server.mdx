---
title: Sunucu
description: bettercode sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`bettercode serve` komutu, bettercode istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `bettercode.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `BETTERCODE_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `bettercode` değeridir; değiştirmek isterseniz `BETTERCODE_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `bettercode serve` hem de `bettercode web` için geçerlidir.
...
`bettercode` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
bettercode ile programatik etkileşim için bettercode sunucusunu kullanın.
:::

Bu mimari, bettercode'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`bettercode serve` ile bağımsız bir sunucu başlatabilirsiniz. bettercode TUI açıksa, `bettercode serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum bettercode [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
bettercode sunucusu aşağıdaki API'leri sunar.
