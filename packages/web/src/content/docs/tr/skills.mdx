...
Ajan becerileri, bettercode'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
bettercode su konumlari tarar:

- Proje konfigurasyonu: `.bettercode/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/bettercode/skills/<name>/SKILL.md`
...
Proje yerel yollarinda bettercode, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.bettercode/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/bettercode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.bettercode/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
bettercode, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`bettercode.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`bettercode.json` icinde):
