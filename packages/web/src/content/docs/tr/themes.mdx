...
bettercode ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak bettercode kendi `bettercode` temasını kullanır.
...
bettercode birden fazla yerleşik temayla gelir.
...
- bettercode'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
bettercode, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/bettercode/themes/*.json` veya `$XDG_CONFIG_HOME/bettercode/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.bettercode/themes/*.json`
4. **Geçerli çalışma dizini** - `./.bettercode/themes/*.json`
