---
title: TUI
description: Using the BetterCode terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

BetterCode provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running BetterCode starts the TUI for the current directory.
...
When using the BetterCode TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to BetterCode. Allows you to select from available providers and add their API keys.
...
Exit BetterCode. _Aliases_: `/quit`, `/q`
...
This is separate from `bettercode.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long BetterCode waits after the leader key. Defaults to `2000`.
...
Use `BETTERCODE_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `bettercode.default`.
