...
import { createOpencode } from "@bettercode/sdk"
...
`)
  ...
  const bettercode = await createOpencode({ port: 0 })
  ...
  try {
    ...
    const session = await bettercode.client.session.create()
    const result = await bettercode.client.session
      .prompt({
