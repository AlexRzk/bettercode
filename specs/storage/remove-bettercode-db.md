# Remove `packages/bettercode/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/bettercode/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the bettercode legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/bettercode/src/storage/db.ts` are concentrated in 22 source files:

- `packages/bettercode/src/account/repo.ts`
- `packages/bettercode/src/cli/cmd/db.ts`
- `packages/bettercode/src/cli/cmd/import.ts`
- `packages/bettercode/src/cli/cmd/stats.ts`
- `packages/bettercode/src/control-plane/workspace.ts`
- `packages/bettercode/src/index.ts`
- `packages/bettercode/src/node.ts`
- `packages/bettercode/src/permission/index.ts`
- `packages/bettercode/src/project/project.ts`
- `packages/bettercode/src/server/projectors.ts`
- `packages/bettercode/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/bettercode/src/server/shared/fence.ts`
- `packages/bettercode/src/session/message-v2.ts`
- `packages/bettercode/src/session/projectors.ts`
- `packages/bettercode/src/session/prompt.ts`
- `packages/bettercode/src/session/session.ts`
- `packages/bettercode/src/session/todo.ts`
- `packages/bettercode/src/share/share-next.ts`
- `packages/bettercode/src/storage/db.ts`
- `packages/bettercode/src/sync/index.ts`
- `packages/bettercode/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy bettercode database wrapper; `packages/bettercode/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the bettercode wrapper.
...
Status: Completed. `SyncEvent` and the bettercode projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy bettercode database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy bettercode database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy bettercode database wrapper.
...
All migration groups are complete or superseded. `packages/bettercode/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No bettercode data-migration group remains.

The previous bettercode `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate bettercode data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/bettercode`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/bettercode/src`
- `bun typecheck` from `packages/bettercode`
- Relevant package tests from `packages/bettercode`, not the repo root
