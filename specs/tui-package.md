...
Move the canonical BetterCode terminal application from
`packages/bettercode/src/cli/cmd/tui` into a self-contained workspace package while
...
generic presentation libraries. It must not depend on `packages/bettercode`,
`packages/cli`, or `@bettercode/core`.

The SDK is the TUI's BetterCode boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/bettercode`.
- Do not replace private `packages/bettercode` imports with `@bettercode/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@bettercode/tui`.
...
- BetterCode domain data displayed by the TUI
...
```text
packages/bettercode/src/cli/cmd/tui
```

Its private dependency on `packages/bettercode` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/bettercode/src/*`.
...
Status: Completed. The private `@bettercode/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@bettercode/tui`.
...
- The package has no dependency on `bettercode`, `@bettercode/cli`, or
  `@bettercode/core`.
...
primitives now live in `@bettercode/tui`. The legacy host consumes explicit
...
and build inputs through `@bettercode/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads BetterCode globals or process state;
...
Replace process-global BetterCode state with resolved TUI inputs.
...
`@bettercode/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/bettercode` initially.
...
canonical in `@bettercode/tui`. Configured references resolve through the new
...
adapters rather than BetterCode domain implementations.

Make the SDK the only BetterCode domain boundary used by the TUI.
...
- Domain-facing TUI code imports BetterCode data and operations only from
  `@bettercode/sdk`.
...
`@bettercode/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/bettercode`.
...
- The package has no imports from `packages/bettercode`, `packages/cli`, or
  `@bettercode/core`.
...
public `@bettercode/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/bettercode`.
- Keep the legacy embedded worker and server startup in `packages/bettercode`.
- Change those adapters to load config, create transport inputs, and call the
  public `@bettercode/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/bettercode`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@bettercode/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@bettercode/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for BetterCode domain state.
...
cd packages/bettercode && bun typecheck
...
rg '@bettercode/core|packages/bettercode|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/bettercode/src packages/bettercode/test
```
...
- Build the current-platform `packages/bettercode` binary.
