...
Both `action` and `resource` use bettercode's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.bettercode` files is intentionally deferred until `.bettercode` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether bettercode permits an operation through its managed execution paths.
