...
The bettercode server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@bettercode/llm` tool loop after replacing its
...
- plugins get global "bettercode" instance like in that post i showed
- bettercode instance has stuff like `bettercode.session.prompt()` or
  `bettercode.tool.register({...})`
...
- decide whether to preserve deprecated `@bettercode/llm` orchestration exports
